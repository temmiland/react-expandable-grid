import CJS_COMPAT_NODE_URL_bvy9c8jbty from 'node:url';
import CJS_COMPAT_NODE_PATH_bvy9c8jbty from 'node:path';
import CJS_COMPAT_NODE_MODULE_bvy9c8jbty from "node:module";

var __filename = CJS_COMPAT_NODE_URL_bvy9c8jbty.fileURLToPath(import.meta.url);
var __dirname = CJS_COMPAT_NODE_PATH_bvy9c8jbty.dirname(__filename);
var require = CJS_COMPAT_NODE_MODULE_bvy9c8jbty.createRequire(import.meta.url);

// ------------------------------------------------------------
// end of CJS compatibility banner, injected by Storybook's esbuild configuration
// ------------------------------------------------------------
import "./_node-chunks/chunk-2ZYOPMW6.js";
...
// src/csf-plugin/index.ts
import { dirname as dirname2, resolve as resolve2 } from "node:path";
import { fileURLToPath as fileURLToPath2 } from "node:url";
import { createUnplugin } from "unplugin";

// src/csf-plugin/constants.ts
var STORIES_REGEX = /(?<!node_modules.*)\.(story|stories)\.[tj]sx?$/;

// src/csf-plugin/rollup-based-plugin.ts
import { readFile } from "node:fs/promises";
import { enrichCsf, formatCsf, loadCsf } from "storybook/internal/csf-tools";
import { logger } from "storybook/internal/node-logger";
async function transformCsf(code, id, options) {
  if (!STORIES_REGEX.test(id))
    return;
  let sourceCode = await readFile(id, "utf-8");
  try {
    let makeTitle = (userTitle) => userTitle || "default", csf = loadCsf(code, { makeTitle }).parse(), csfSource = loadCsf(sourceCode, { makeTitle }).parse();
    ...
    return message.startsWith("CSF:") || logger.warn(message), code;
  }
}
...
// src/csf-plugin/index.ts
var webpackLoader = resolve2(
  dirname2(fileURLToPath2(import.meta.resolve("@storybook/addon-docs/package.json"))),
  "dist/csf-plugin/webpack-loader.js"
), unpluginFactory = (options) => ({
  name: "unplugin-csf",
  rollup: {
...
}), unplugin = createUnplugin(unpluginFactory), { webpack, vite } = unplugin;
...
var getResolvedReact = async (options) => {
  ...
  return {
    ...
    mdx: resolvedReact2.mdx ?? fileURLToPath3(import.meta.resolve("@mdx-js/react"))
  };
};
async function webpack2(webpackConfig = {}, options) {
  let { module = {} } = webpackConfig, { csfPluginOptions = {}, mdxPluginOptions = {} } = options, enrichCsf2 = await options.presets.apply("experimental_enrichCsf"), rehypeSlug = (await import("./_node-chunks/rehype-slug-VQBJLUQ6.js")).default, rehypeExternalLinks = (await import("./_node-chunks/rehype-external-links-FKYL2JPG.js")).default, mdxLoaderOptions = await options.presets.apply("mdxLoaderOptions", {
    ...mdxPluginOptions,
    mdxCompileOptions: {
  ...
  logger3.info("Addon-docs: using MDX3");
    ...
    module: {
      ...
      rules: [
        ...module.rules || [],
        {
          ...
          use: [
            {
              loader: fileURLToPath3(import.meta.resolve("@storybook/addon-docs/mdx-loader")),
...
var docs = (input = {}, options) => {
  ...
  let { plugins = [] } = config, { csfPluginOptions = {} } = options, { mdxPlugin } = await import("./_node-chunks/mdx-plugin-NDXQWJZM.js"), enrichCsf2 = await options.presets.apply("experimental_enrichCsf"), { react, reactDom, mdx } = await getResolvedReact(options), packageDeduplicationPlugin = {
  ...
  return csfPluginOptions && plugins.unshift(
    vite({
      ...csfPluginOptions,
      enrichCsf: enrichCsf2
    })
  ), plugins.unshift(packageDeduplicationPlugin), plugins.unshift(mdxPlugin(options)), {
    ...config,
    plugins
  };
}, webpackX = webpack2, docsX = docs, resolvedReact = async (existing) => ({
