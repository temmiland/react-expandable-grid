import CJS_COMPAT_NODE_URL_xsxr456oav from 'node:url';
import CJS_COMPAT_NODE_PATH_xsxr456oav from 'node:path';
import CJS_COMPAT_NODE_MODULE_xsxr456oav from "node:module";

var __filename = CJS_COMPAT_NODE_URL_xsxr456oav.fileURLToPath(import.meta.url);
var __dirname = CJS_COMPAT_NODE_PATH_xsxr456oav.dirname(__filename);
var require = CJS_COMPAT_NODE_MODULE_xsxr456oav.createRequire(import.meta.url);
...
var docsUrl = (ruleName) => `https://github.com/storybookjs/storybook/blob/next/code/lib/eslint-plugin/docs/rules/${ruleName}.md`, getMetaObjectExpression = (node, context) => {
  ...
  if (isIdentifier(meta)) {
    let scope = sourceCode.getScope ? sourceCode.getScope(node) : context.getScope(), variable = ASTUtils.findVariable(scope, meta.name), decl = variable && variable.defs.find((def) => isVariableDeclarator(def.node));
        ...
        suggest: [
          {
            ...
            *fix(fixer) {
              ...
              if (scope) {
                let variable = ASTUtils.findVariable(scope, name), referenceCount = variable?.references?.length || 0;
