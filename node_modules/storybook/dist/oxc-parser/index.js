...
async function parseDefaultImports(filePath, source) {
  let parseResult;
  try {
    parseResult = oxcRawParseSync(filePath, source);
  } catch {
  ...
  for (let staticImport of moduleInfo.staticImports)
    for (let entry of staticImport.entries)
      entry.isType || entry.importName.kind !== "Default" || map.set(entry.localName.value, staticImport.moduleRequest.value);
  return map;
}
