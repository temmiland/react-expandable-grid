...
function connectCommandTransport(context) {
  let {
    ...
    if (!dispatchesLocally(invoke.commandName)) {
      !delegated && invoke.clientId !== ownClientId && channel.emit(SERVICE_COMMAND_UNHANDLED, {
        serviceId,
  ...
  channel.on(SERVICE_COMMAND_INVOKE, onInvoke), channel.on(SERVICE_COMMAND_RESULT, onResult), channel.on(SERVICE_COMMAND_ERROR, onError), channel.on(SERVICE_COMMAND_ACK, onAck), channel.on(SERVICE_COMMAND_UNHANDLED, onUnhandled);
  ...
  return {
    commands,
    disconnect: () => {
      channel.off(SERVICE_COMMAND_INVOKE, onInvoke), channel.off(SERVICE_COMMAND_RESULT, onResult), channel.off(SERVICE_COMMAND_ERROR, onError), channel.off(SERVICE_COMMAND_ACK, onAck), channel.off(SERVICE_COMMAND_UNHANDLED, onUnhandled);
      for (let [, entry] of pending)
...
function connectUnknownServiceReporter(context) {
  let { channel, isServiceRegistered, isDelegated } = context, ownClientId = generateClientId(), onInvoke = (payload) => {
    let parsed = safeParse(commandInvokeSchema, payload);
    !parsed.success || isDelegated() || isServiceRegistered(parsed.output.serviceId) || channel.emit(SERVICE_COMMAND_UNHANDLED, {
      serviceId: parsed.output.serviceId,
      callId: parsed.output.callId,
      clientId: ownClientId
    });
  };
  return channel.on(SERVICE_COMMAND_INVOKE, onInvoke), () => {
...
function ensureUnknownServiceReporter(channel) {
  let inventory = getInventory();
  inventory.unknownServiceReporter?.channel !== channel && (inventory.unknownServiceReporter?.disconnect(), inventory.unknownServiceReporter = {
    channel,
...
function registerService(definition, registration, { relay = !1, staticLoader } = {}) {
  ...
  getInventory().everRegisteredIds.add(definition.id), ensureUnknownServiceReporter(channel);
