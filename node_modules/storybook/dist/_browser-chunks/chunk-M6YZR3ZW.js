...
import {
  SNIPPET_RENDERED
} from "./chunk-RDNWKK3U.js";
import {
  combineParameters,
  inferArgTypes,
  inferControls
} from "./chunk-YX25HBUE.js";
import {
  mapValues
} from "./chunk-7PIQR4A3.js";
import {
  isEqual,
  isPlainObject
...
var argTypesForValidation = (story) => global2.FEATURES?.experimentalDocgenServer ? inferArgTypes({
  id: story.id,
  argTypes: story.argTypes,
  initialArgs: story.initialArgs
}) : story.argTypes, ArgsStore = class {
  constructor() {
  ...
  updateFromDelta(story, delta) {
    let validatedDelta = validateOptions(delta, argTypesForValidation(story));
    this.argsByStoryId[story.id] = combineArgs(this.argsByStoryId[story.id], validatedDelta);
  }
  updateFromPersisted(story, persisted) {
    let mappedPersisted = mapArgsToTypes(persisted, argTypesForValidation(story));
...
function preparePartialAnnotations(storyAnnotations, componentAnnotations, projectAnnotations) {
  ...
  contextForEnhancers.argTypes = argTypesEnhancers.filter((enhancer) => !(global3.FEATURES?.experimentalDocgenServer && enhancer.secondPass)).reduce(
...
var FREEZE_STYLE_ID = "storybook-freeze-after-finished", PRE_FREEZE_STYLE_ID = "storybook-freeze-end-frame-preload", tryReplaceProperty = (target, key, value) => {
  ...
  let windowRef = global6.window, documentRef = global6.document;
...
var EMBED_STYLE_ID = "storybook-embed-sizing", EMBED_UI_STYLE_ID = "storybook-embed-ui", IGNORE_TAGS = /* @__PURE__ */ new Set([
  ...
  let { getViewport } = options, windowRef = global7.window, documentRef = global7.document;
...
var PreviewWithSelection = class extends Preview {
  ...
  onNavigateUrl(url) {
    url.startsWith("#") && this.view.scrollToAnchor?.(url);
  }
  async setInitialGlobals() {
    if (!this.storyStoreValue)
...
// src/preview-api/modules/preview-web/UrlStore.ts
var { history, document: document2 } = global8, PATH_REGEX = /^\/(story|docs)\/(.+)/;
...
var { document: document3 } = global9, PREPARING_DELAY = 100, Mode = /* @__PURE__ */ ((Mode2) => (Mode2.MAIN = "MAIN", Mode2.NOPREVIEW = "NOPREVIEW", Mode2.PREPARING_STORY = "PREPARING_STORY", Mode2.PREPARING_DOCS = "PREPARING_DOCS", Mode2.ERROR = "ERROR", Mode2))(Mode || {}), classes = {
  ...
  scrollToAnchor(hash) {
    document3.getElementById(decodeURIComponent(hash.substring(1)))?.scrollIntoView({ behavior: "smooth" });
  }
...
var PreviewWeb = class extends PreviewWithSelection {
  constructor(importFn, getProjectAnnotations) {
    ...
    this.importFn = importFn;
    this.getProjectAnnotations = getProjectAnnotations;
    global10.__STORYBOOK_PREVIEW__ = this;
  }
};

// src/preview-api/modules/preview-web/simulate-pageload.ts
import { global as global11 } from "@storybook/global";
var { document: document4 } = global11, runScriptTypes = [
...
async function emitTransformCode(source, context, warning) {
