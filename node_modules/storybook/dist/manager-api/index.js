...
} from "../_browser-chunks/chunk-MOVZJCSM.js";
import {
  ...
  toStatusValue
} from "../_browser-chunks/chunk-D6P6QKH4.js";
import {
  countBy,
  dequal,
  getService,
...
import "../_browser-chunks/chunk-XJE6FSX5.js";
import {
  BUILT_IN_FILTERS,
  Tag,
  USER_TAG_FILTER,
...
import "../_browser-chunks/chunk-VM65THJG.js";
import {
  stringify
...
var { document: document2 } = global2, isFunction = (val) => typeof val == "function", ActiveTabs = {
...
}, isDesktopViewport = () => global2.matchMedia?.("(min-width: 600px)")?.matches ?? !0, init5 = ({ store: store2, provider, singleStory }) => {
  let api = {
    ...
    toggleNav(nextState) {
      return isDesktopViewport() ? store2.setState(
        ...
        layout: {
          ...state.layout,
          showMobileNavigation: typeof nextState == "boolean" ? nextState : !state.layout.showMobileNavigation
        }
      }));
    },
  ...
  return persisted.layout && (persisted.layout = { ...persisted.layout, showMobileNavigation: !1 }), provider.channel?.on(SET_CONFIG2, () => {
    ...
    handleShortcutFeature(feature, event) {
      ...
      if (!(isSidebarShortcutBlocked && isSidebarShortcutFeature))
        switch (event?.preventDefault && event.preventDefault(), feature) {
          ...
          case "toggleNav": {
            let isDesktop = isDesktopViewport(), wasNavShown = fullAPI.getIsNavShown(), sidebarElement = document3.getElementById(focusableUIElements.sidebarRegion);
...
var FILTER_KEYS = {
  ...
  let navigateWithQueryParams = (path, options) => {
    ...
    selectStory: (titleOrId = void 0, name = void 0, options = {}) => {
      let { ref, scrollTo } = options, { storyId, index, filteredIndex, refs, settings } = store2.getState(), gotoStory = (entry) => entry?.type === "docs" || entry?.type === "story" ? (store2.setState({ settings: { ...settings, lastTrackedStoryId: entry.id } }), navigateWithQueryParams(
    ...
    experimental_setFilters: async (filters) => {
      await store2.setState((state) => ({ filters: { ...state.filters, ...filters } }));
  ...
  provider.channel?.on(
    ...
    let { ref } = getEventMetadata(this, fullAPI);
    api.setPreviewInitialized(ref);
  }), provider.channel?.on(SET_CONFIG3, async () => {
...
var ManagerContext = createContext({ api: void 0, state: initial_state_default({}) }), combineParameters = (...parameterSets) => noArrayMerge({}, ...parameterSets), ManagerProvider = class extends Component {
  ...
  static getDerivedStateFromProps(props, state) {
    let locationSearchChanged = state.location?.search !== props.location?.search, locationHashChanged = state.location?.hash !== props.location?.hash;
    return state.path !== props.path || locationSearchChanged || locationHashChanged ? {
      ...state,
