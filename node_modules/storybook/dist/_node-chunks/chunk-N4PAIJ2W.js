...
import {
  getSessionId
} from "./chunk-2BMSH5CU.js";
import {
  getMonorepoType,
  version
} from "./chunk-VBSZV6KY.js";
import {
  detectAgent
    ...
    module.exports = function(fetch2, defaults) {
      ...
      return defaults = Object.assign(baseDefaults, defaults), function(input, init) {
        ...
        return new Promise(function(resolve2, reject) {
          var wrappedFetch = function(attempt2) {
            ...
            fetch2(_input, init).then(function(response) {
              if (Array.isArray(retryOn) && retryOn.indexOf(response.status) === -1)
                ...
                try {
                  return Promise.resolve(retryOn(attempt2, null, response)).then(function(retryOnResponse) {
                    retryOnResponse ? retry2(attempt2, null, response) : resolve2(response);
                  }).catch(reject);
                } catch (error) {
                  reject(error);
                }
              else
                attempt2 < retries ? retry2(attempt2, null, response) : resolve2(response);
            }).catch(function(error) {
              if (typeof retryOn == "function")
                try {
                  Promise.resolve(retryOn(attempt2, error, null)).then(function(retryOnResponse) {
                    retryOnResponse ? retry2(attempt2, error, null) : reject(error);
              ...
              else attempt2 < retries ? retry2(attempt2, error, null) : reject(error);
            });
          };
          function retry2(attempt2, error, response) {
            var delay = typeof retryDelay == "function" ? retryDelay(attempt2, error, response) : retryDelay;
            setTimeout(function() {
              wrappedFetch(++attempt2);
            }, delay);
          }
          wrappedFetch(0);
        });
...
// src/telemetry/get-has-module-federation.ts
var MODULE_FEDERATION_PACKAGES = ["@module-federation/*", "@originjs/vite-plugin-federation"];
function getHasModuleFederation(packageJson) {
  let allDependencies = {
    ...packageJson?.dependencies,
    ...packageJson?.devDependencies
  };
  return Object.keys(allDependencies).some(
    (depName) => matchesPackagePattern(depName, MODULE_FEDERATION_PACKAGES)
  );
}
...
// src/telemetry/get-has-next-custom-webpack.ts
...
function getHasNextCustomWebpack(projectRoot) {
  for (let configFile of NEXT_CONFIG_FILES) {
    let configPath = join(projectRoot, configFile);
...
// src/telemetry/get-renderer-packages.ts
var RENDERER_RUNTIME_PACKAGES = {
...
function getRendererPackages(renderer) {
  return renderer ? RENDERER_RUNTIME_PACKAGES[renderer] ?? [] : [];
}

// src/telemetry/get-has-turbopack.ts
function getHasTurbopack(packageJson) {
  let nextCommand = /(?<![\w/.-])next\s+(?:dev|build)(?![\w.-])/, scripts = Object.values(packageJson?.scripts ?? {}).filter(
    (script) => typeof script == "string" && nextCommand.test(script)
  );
  if (scripts.length !== 0) {
...
var getActualPackageVersions = async (packages) => {
  ...
  let resolvedPackageJsonPath = resolvePackageJsonPath(packageName);
...
}, resolvePackageJsonPath = (packageName) => {
  let projectRequire = createRequire(join2(process.cwd(), "package.json"));
  return attempt(() => projectRequire.resolve(`${packageName}/package.json`)) ?? attempt(() => up({ cwd: projectRequire.resolve(packageName) }) || void 0) ?? findPackageJsonInNodeModules(packageName);
}, findPackageJsonInNodeModules = (packageName) => {
  for (let dir = process.cwd(); ; dir = dirname(dir)) {
    let candidate = join2(dir, "node_modules", packageName, "package.json");
...
async function analyzeEcosystemPackages(packageJson) {
  let allDependencies = {
  ...
  }, pickResolvedDepsObject = async (packages) => {
    let result = Object.fromEntries(
      await Promise.all(
    ...
    return Object.keys(result).length === 0 ? null : result;
  }, testPackages = await pickResolvedDepsObject(TEST_PACKAGES), bundlerPackages = await pickResolvedDepsObject(BUNDLER_PACKAGES), stylingPackages = pickDepsObject(STYLING_PACKAGES), stateManagementPackages = pickDepsObject(STATE_MANAGEMENT_PACKAGES), dataFetchingPackages = pickDepsObject(DATA_FETCHING_PACKAGES), uiLibraryPackages = pickDepsObject(UI_LIBRARY_PACKAGES), i18nPackages = pickDepsObject(I18N_PACKAGES), routerPackages2 = pickDepsObject(ROUTER_PACKAGES);
  return {
    ...testPackages && { testPackages },
    ...bundlerPackages && { bundlerPackages },
...
var metaFrameworks = {
  ...
  metadata.knownPackages = await analyzeEcosystemPackages(packageJson), metadata.hasRouterPackage = getHasRouterPackage(packageJson), metadata.hasTurbopack = getHasTurbopack(packageJson), metadata.hasModuleFederation = getHasModuleFederation(packageJson);
  ...
  metadata.hasCustomBabel = !!mainConfig.babel, metadata.hasCustomWebpack = !!mainConfig.webpackFinal || !!allDependencies.next && getHasNextCustomWebpack(dirname2(packageJsonPath)), metadata.hasCustomVite = !!mainConfig.viteFinal, metadata.hasStaticDirs = !!mainConfig.staticDirs, typeof mainConfig.typescript == "object" && (metadata.typescriptOptions = mainConfig.typescript);
  let frameworkInfo = await getFrameworkInfo(mainConfig, configDir), rendererPackages = Object.fromEntries(
    await Promise.all(
      getRendererPackages(frameworkInfo.renderer).map(async (packageName) => {
        let { version: version2 } = await getActualPackageVersion(packageName);
        return [packageName, version2 || "unknown"];
      })
    )
  );
  Object.keys(rendererPackages).length > 0 && (metadata.knownPackages = { ...metadata.knownPackages, rendererPackages }), typeof mainConfig.refs == "object" && (metadata.refCount = Object.keys(mainConfig.refs).length), typeof mainConfig.features == "object" && (metadata.features = mainConfig.features);
  let addons = {};
  ...
  let portableStoriesFileCount = await getPortableStoriesFileCount(), applicationFileCount = await getApplicationFileCount(dirname2(packageJsonPath));
  return {
    ...metadata,
    ...frameworkInfo,
...
async function hashMainConfig(configDir) {
  try {
    ...
    if (!mainPath || !existsSync3(mainPath))
      return "missing";
...
function resolveDefaultConfigDir(packageJson) {
  ...
  return fromScript || (existsSync3(resolve(RN_STORYBOOK_DIR)) ? RN_STORYBOOK_DIR : ".storybook");
}
...
var retryingFetch = (0, import_fetch_retry.default)(fetch), URL = process.env.STORYBOOK_TELEMETRY_URL || "https://storybook.js.org/event-log", tasks = [], addToGlobalContext = (key, value) => {
  ...
  return retryingFetch(URL, {
    ...
    retryDelay: (attempt2) => 2 ** attempt2 * (typeof options?.retryDelay == "number" && !Number.isNaN(options?.retryDelay) ? options.retryDelay : 1e3),
    retryOn: (attempt2, error, response) => signal.aborted || attempt2 >= maxRetries ? !1 : !!error || response?.status === 503 || response?.status === 504,
    signal
  });
};
