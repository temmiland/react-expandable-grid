...
import {
  require_picomatch
} from "./chunk-AWDYUSA7.js";
import {
  versions_default
} from "./chunk-XSINJFVF.js";
import {
  logger
} from "./chunk-2WYCRIJA.js";
import {
  BUN_LOCKFILE,
  BUN_LOCKFILE_BINARY,
  NPM_LOCKFILE,
...
var require_createNode = __commonJS({
  "../../node_modules/yaml/dist/doc/createNode.js"(exports) {
    ...
    function findTagObject(value, tagName, tags) {
      if (tagName) {
        let match = tags.filter((t4) => t4.tag === tagName), tagObj = match.find((t4) => !t4.format) ?? match[0];
        if (!tagObj)
    ...
    function createNode(value, tagName, ctx) {
      ...
      if (identity.isPair(value)) {
        let map2 = ctx.schema[identity.MAP].createNode?.(ctx.schema, null, ctx);
...
${indent}`);
    ...
    function stringifyString(item, ctx, onComment, onChompKeep) {
      ...
      if (res === null) {
        let { defaultKeyType, defaultStringType } = ctx.options, t4 = implicitKey && defaultKeyType || defaultStringType;
        if (res = _stringify(t4), res === null)
...
var require_stringify = __commonJS({
  "../../node_modules/yaml/dist/stringify/stringify.js"(exports) {
    ...
    function getTagObject(tags, item) {
      if (item.tag) {
        let match = tags.filter((t4) => t4.tag === item.tag);
        if (match.length > 0)
      ...
      if (identity.isScalar(item)) {
        ...
        if (match.length > 1) {
          let testMatch = match.filter((t4) => t4.test);
          testMatch.length > 0 && (match = testMatch);
        }
        tagObj = match.find((t4) => t4.format === item.format) ?? match.find((t4) => !t4.format);
      } else
        obj = item, tagObj = tags.find((t4) => t4.nodeClass && obj instanceof t4.nodeClass);
      if (!tagObj) {
  ...
  "../../node_modules/yaml/dist/schema/yaml-1.1/merge.js"(exports) {
    ...
    function mergeValue(ctx, map2, value) {
      ...
      for (let [key, value2] of srcMap)
        map2 instanceof Map ? map2.has(key) || map2.set(key, value2) : map2 instanceof Set ? map2.add(key) : Object.prototype.hasOwnProperty.call(map2, key) || Object.defineProperty(map2, key, {
          value: value2,
          writable: !0,
          enumerable: !0,
          configurable: !0
    ...
    function addPairToJSMap(ctx, map2, { key, value }) {
      if (identity.isNode(key) && key.addToJSMap)
        ...
        if (map2 instanceof Map)
          ...
          stringKey in map2 ? Object.defineProperty(map2, stringKey, {
            value: jsValue,
            writable: !0,
            enumerable: !0,
            configurable: !0
  ...
  "../../node_modules/yaml/dist/nodes/YAMLMap.js"(exports) {
    ...
    var YAMLMap = class extends Collection.Collection {
      ...
      static from(schema, obj, ctx) {
        let { keepUndefined, replacer } = ctx, map2 = new this(schema), add = (key, value) => {
          if (typeof replacer == "function")
            value = replacer.call(obj, key, value);
          else if (Array.isArray(replacer) && !replacer.includes(key))
            return;
          (value !== void 0 || keepUndefined) && map2.items.push(Pair.createPair(key, value, ctx));
        };
       ...
       */
      toJSON(_, ctx, Type) {
        let map2 = Type ? new Type() : ctx?.mapAsMap ? /* @__PURE__ */ new Map() : {};
        ctx?.onCreate && ctx.onCreate(map2);
...
var require_map = __commonJS({
  "../../node_modules/yaml/dist/schema/common/map.js"(exports) {
    "use strict";
    var identity = require_identity(), YAMLMap = require_YAMLMap(), map2 = {
      collection: "map",
      default: !0,
      nodeClass: YAMLMap.YAMLMap,
      tag: "tag:yaml.org,2002:map",
      resolve(map3, onError) {
        return identity.isMap(map3) || onError("Expected a mapping for this tag"), map3;
      },
...
var require_schema = __commonJS({
  "../../node_modules/yaml/dist/schema/core/schema.js"(exports) {
    "use strict";
    var map2 = require_map(), _null = require_null(), seq = require_seq(), string = require_string(), bool = require_bool(), float = require_float(), int = require_int(), schema = [
      map2.map,
      seq.seq,
      string.string,
      _null.nullTag,
...
var require_schema2 = __commonJS({
  "../../node_modules/yaml/dist/schema/json/schema.js"(exports) {
    "use strict";
    var Scalar = require_Scalar(), map2 = require_map(), seq = require_seq();
    ...
    }, schema = [map2.map, seq.seq].concat(jsonScalars, jsonError);
    exports.schema = schema;
  }
});
    ...
    var identity = require_identity(), toJS = require_toJS(), YAMLMap = require_YAMLMap(), YAMLSeq = require_YAMLSeq(), pairs = require_pairs(), YAMLOMap = class _YAMLOMap extends YAMLSeq.YAMLSeq {
      ...
      toJSON(_, ctx) {
        ...
        for (let pair of this.items) {
          let key, value;
          if (identity.isPair(pair) ? (key = toJS.toJS(pair.key, "", ctx), value = toJS.toJS(pair.value, key, ctx)) : key = toJS.toJS(pair, "", ctx), map2.has(key))
...
var require_schema3 = __commonJS({
  "../../node_modules/yaml/dist/schema/yaml-1.1/schema.js"(exports) {
    "use strict";
    var map2 = require_map(), _null = require_null(), seq = require_seq(), string = require_string(), binary = require_binary(), bool = require_bool2(), float = require_float2(), int = require_int2(), merge = require_merge(), omap = require_omap(), pairs = require_pairs(), set = require_set(), timestamp = require_timestamp(), schema = [
      map2.map,
      seq.seq,
      string.string,
      _null.nullTag,
...
var require_Schema = __commonJS({
  "../../node_modules/yaml/dist/schema/Schema.js"(exports) {
    "use strict";
    var identity = require_identity(), map2 = require_map(), seq = require_seq(), string = require_string(), tags = require_tags(), sortMapEntriesByKey = (a, b) => a.key < b.key ? -1 : a.key > b.key ? 1 : 0, Schema = class _Schema {
      constructor({ compat, customTags, merge, resolveKnownTags, schema, sortMapEntries, toStringDefaults }) {
        this.compat = Array.isArray(compat) ? tags.getTags(compat, "compat") : compat ? tags.getTags(null, compat) : null, this.name = typeof schema == "string" && schema || "core", this.knownTags = resolveKnownTags ? tags.coreKnownTags : {}, this.tags = tags.getTags(customTags, this.name, merge), this.toStringOptions = toStringDefaults ?? null, Object.defineProperty(this, identity.MAP, { value: map2.map }), Object.defineProperty(this, identity.SCALAR, { value: string.string }), Object.defineProperty(this, identity.SEQ, { value: seq.seq }), this.sortMapEntries = typeof sortMapEntries == "function" ? sortMapEntries : sortMapEntries === !0 ? sortMapEntriesByKey : null;
      }
      clone() {
...
var require_resolve_block_map = __commonJS({
  "../../node_modules/yaml/dist/compose/resolve-block-map.js"(exports) {
    ...
    function resolveBlockMap({ composeNode, composeEmptyNode }, ctx, bm, onError, tag) {
      let NodeClass = tag?.nodeClass ?? YAMLMap.YAMLMap, map2 = new NodeClass(ctx.schema);
      ...
      for (let collItem of bm.items) {
        ...
        if (implicitKey) {
          if (key && (key.type === "block-seq" ? onError(offset, "BLOCK_AS_IMPLICIT_KEY", "A block sequence may not be used as an implicit map key") : "indent" in key && key.indent !== bm.indent && onError(offset, "BAD_INDENT", startColMsg)), !keyProps.anchor && !keyProps.tag && !sep3) {
            commentEnd = keyProps.end, keyProps.comment && (map2.comment ? map2.comment += `
` + keyProps.comment : map2.comment = keyProps.comment);
            continue;
          }
        ...
        ctx.schema.compat && utilFlowIndentCheck.flowIndentCheck(bm.indent, key, onError), ctx.atKey = !1, utilMapIncludes.mapIncludes(ctx, map2.items, keyNode) && onError(keyStart, "DUPLICATE_KEY", "Map keys must be unique");
        ...
        if (offset = valueProps.end, valueProps.found) {
          ...
          ctx.options.keepSourceTokens && (pair.srcToken = collItem), map2.items.push(pair);
        } else {
...
` + valueProps.comment : keyNode.comment = valueProps.comment);
      ...
      return commentEnd && commentEnd < offset && onError(commentEnd, "IMPOSSIBLE", "Map comment with trailing content"), map2.range = [bm.offset, offset, commentEnd ?? offset], map2;
    }
...
` + valueProps.comment : keyNode.comment = valueProps.comment);
          ...
          if (ctx.options.keepSourceTokens && (pair.srcToken = collItem), isMap) {
            let map2 = coll;
            utilMapIncludes.mapIncludes(ctx, map2.items, keyNode) && onError(keyStart, "DUPLICATE_KEY", "Map keys must be unique"), map2.items.push(pair);
          } else {
            let map2 = new YAMLMap.YAMLMap(ctx.schema);
            map2.flow = !0, map2.items.push(pair);
            let endRange = (valueNode ?? keyNode).range;
            map2.range = [keyNode.range[0], endRange[1], endRange[2]], coll.items.push(map2);
          }
    ...
    var Parser = class {
      ...
      *scalar(scalar) {
        if (this.type === "map-value-ind") {
          ...
          this.onKeyLine = !0, this.stack[this.stack.length - 1] = map2;
        } else
...
`, nl) + 1;
      ...
      *blockMap(map2) {
        let it = map2.items[map2.items.length - 1];
        switch (this.type) {
          case "newline":
            if (this.onKeyLine = !1, it.value) {
              let end = "end" in it.value ? it.value.end : void 0;
              (Array.isArray(end) ? end[end.length - 1] : void 0)?.type === "comment" ? end?.push(this.sourceToken) : map2.items.push({ start: [this.sourceToken] });
          ...
          case "comment":
            if (it.value)
              ...
              if (this.atIndentedComment(it.start, map2.indent)) {
                let end = map2.items[map2.items.length - 2]?.value?.end;
                if (Array.isArray(end)) {
                  Array.prototype.push.apply(end, it.start), end.push(this.sourceToken), map2.items.pop();
                  return;
                }
              }
        ...
        if (this.indent >= map2.indent) {
          let atMapIndent = !this.onKeyLine && this.indent === map2.indent, atNextItem = atMapIndent && (it.sep || it.explicitKey) && this.type !== "seq-item-ind", start = [];
            ...
            for (let i = 0; i < it.sep.length; ++i) {
              ...
              switch (st.type) {
                ...
                case "space":
                  break;
                case "comment":
                  st.indent > map2.indent && (nl.length = 0);
                  break;
                default:
                  nl.length = 0;
              }
          ...
          switch (this.type) {
            case "anchor":
            case "tag":
              atNextItem || it.value ? (start.push(this.sourceToken), map2.items.push({ start }), this.onKeyLine = !0) : it.sep ? it.sep.push(this.sourceToken) : it.start.push(this.sourceToken);
              return;
            case "explicit-key-ind":
              !it.sep && !it.explicitKey ? (it.start.push(this.sourceToken), it.explicitKey = !0) : atNextItem || it.value ? (start.push(this.sourceToken), map2.items.push({ start, explicitKey: !0 })) : this.stack.push({
                type: "block-map",
                offset: this.offset,
                indent: this.indent,
            ...
            case "map-value-ind":
              if (it.explicitKey)
                ...
                it.sep ? it.value || atNextItem ? map2.items.push({ start, key: null, sep: [this.sourceToken] }) : includesToken(it.sep, "map-value-ind") ? this.stack.push({
                  type: "block-map",
                  offset: this.offset,
                  indent: this.indent,
                  items: [{ start: [], key: null, sep: [this.sourceToken] }]
            ...
            case "double-quoted-scalar": {
              ...
              atNextItem || it.value ? (map2.items.push({ start, key: fs, sep: [] }), this.onKeyLine = !0) : it.sep ? this.stack.push(fs) : (Object.assign(it, { key: fs, sep: [] }), this.onKeyLine = !0);
              return;
            }
            default: {
              let bv = this.startBlockValue(map2);
              if (bv) {
                ...
                } else atMapIndent && map2.items.push({ start });
                this.stack.push(bv);
                return;
              }
            }
...
function __rest(s, e) {
  var t4 = {};
  for (var p in s) Object.prototype.hasOwnProperty.call(s, p) && e.indexOf(p) < 0 && (t4[p] = s[p]);
  if (s != null && typeof Object.getOwnPropertySymbols == "function")
    for (var i = 0, p = Object.getOwnPropertySymbols(s); i < p.length; i++)
      e.indexOf(p[i]) < 0 && Object.prototype.propertyIsEnumerable.call(s, p[i]) && (t4[p[i]] = s[p[i]]);
  return t4;
}
...
function __generator(thisArg, body) {
  ...
  function step(op) {
    ...
    for (; _; ) try {
      if (f = 1, y && (t4 = op[0] & 2 ? y.return : op[0] ? y.throw || ((t4 = y.return) && t4.call(y), 0) : y.next) && !(t4 = t4.call(y, op[1])).done) return t4;
      switch (y = 0, t4 && (op = [op[0] & 2, t4.value]), op[0]) {
        case 0:
        case 1:
          t4 = op;
          break;
...
var extendStatics, __assign, init_tslib_es6 = __esm({
  "../../node_modules/@yarnpkg/fslib/node_modules/tslib/tslib.es6.js"() {
    ...
    __assign = function() {
      return __assign = Object.assign || function(t4) {
        for (var s, i = 1, n = arguments.length; i < n; i++) {
          s = arguments[i];
          for (var p in s) Object.prototype.hasOwnProperty.call(s, p) && (t4[p] = s[p]);
        }
        return t4;
...
var require_copyPromise = __commonJS({
  "../../node_modules/@yarnpkg/fslib/lib/algorithms/copyPromise.js"(exports) {
    ...
    var tslib_1 = (init_tslib_es6(), __toCommonJS(tslib_es6_exports)), fs_1 = tslib_1.__importDefault(__require("fs")), constants2 = tslib_1.__importStar(require_constants()), path_1 = require_path(), defaultTime = new Date(constants2.SAFE_TIME * 1e3), LinkStrategy;
...
var require_opendir = __commonJS({
  "../../node_modules/@yarnpkg/fslib/lib/algorithms/opendir.js"(exports) {
    ...
    var tslib_1 = (init_tslib_es6(), __toCommonJS(tslib_es6_exports)), errors = tslib_1.__importStar(require_errors2()), CustomDir = class {
      constructor(path, nextDirent, opts = {}) {
        this.path = path, this.nextDirent = nextDirent, this.opts = opts, this.closed = !1;
      }
      throwIfClosed() {
        if (this.closed)
    ...
    function opendir(fakeFs, path, entries, opts) {
      let nextDirent = () => {
        ...
        return typeof filename > "u" ? null : Object.assign(fakeFs.statSync(fakeFs.pathUtils.join(path, filename)), {
          name: filename
        });
      };
...
var require_FakeFS = __commonJS({
  "../../node_modules/@yarnpkg/fslib/lib/FakeFS.js"(exports) {
    ...
    var os_1 = __require("os"), copyPromise_1 = require_copyPromise(), path_1 = require_path(), FakeFS = class {
          ...
          for (let t4 = 0; t4 <= maxRetries; t4++)
            try {
              ...
              if (error.code !== "EBUSY" && error.code !== "ENOTEMPTY")
                throw error;
              t4 < maxRetries && await new Promise((resolve9) => setTimeout(resolve9, t4 * 100));
            }
        } else
      ...
      async mkdirpPromise(p, { chmod, utimes: utimes2 } = {}) {
        ...
        for (let u = 2; u <= parts.length; ++u) {
          ...
          if (!this.existsSync(subPath)) {
            ...
            if (createdDirectory ?? (createdDirectory = subPath), chmod != null && await this.chmodPromise(subPath, chmod), utimes2 != null)
              await this.utimesPromise(subPath, utimes2[0], utimes2[1]);
            else {
      ...
      mkdirpSync(p, { chmod, utimes: utimes2 } = {}) {
        if (p = this.resolve(p), p === this.pathUtils.dirname(p))
          return;
        let parts = p.split(this.pathUtils.sep), createdDirectory;
        for (let u = 2; u <= parts.length; ++u) {
          ...
          if (!this.existsSync(subPath)) {
            ...
            if (createdDirectory ?? (createdDirectory = subPath), chmod != null && this.chmodSync(subPath, chmod), utimes2 != null)
              this.utimesSync(subPath, utimes2[0], utimes2[1]);
            else {
      ...
      copySync(destination, source, { baseFs = this, overwrite = !0 } = {}) {
        let stat2 = baseFs.lstatSync(source), exists = this.existsSync(destination);
        if (stat2.isDirectory()) {
...
`);
      ...
      async preserveTimePromise(p, cb) {
        let stat2 = await this.lstatPromise(p), result = await cb();
        typeof result < "u" && (p = result), this.lutimesPromise ? await this.lutimesPromise(p, stat2.atime, stat2.mtime) : stat2.isSymbolicLink() || await this.utimesPromise(p, stat2.atime, stat2.mtime);
      }
      async preserveTimeSync(p, cb) {
        let stat2 = this.lstatSync(p), result = cb();
        typeof result < "u" && (p = result), this.lutimesSync ? this.lutimesSync(p, stat2.atime, stat2.mtime) : stat2.isSymbolicLink() || this.utimesSync(p, stat2.atime, stat2.mtime);
      }
    };
...
var require_NodeFS = __commonJS({
  "../../node_modules/@yarnpkg/fslib/lib/NodeFS.js"(exports) {
    ...
    var tslib_1 = (init_tslib_es6(), __toCommonJS(tslib_es6_exports)), fs_1 = tslib_1.__importDefault(__require("fs")), FakeFS_1 = require_FakeFS(), errors_1 = require_errors2(), path_1 = require_path(), NodeFS = class extends FakeFS_1.BasePortableFakeFS {
      ...
      async realpathPromise(p) {
        ...
        }).then((path) => path_1.npath.toPortablePath(path));
      }
      realpathSync(p) {
  ...
  "../../node_modules/@yarnpkg/fslib/lib/algorithms/watchFile/CustomStatWatcher.js"(exports) {
    ...
    var CustomStatWatcher = class _CustomStatWatcher extends events_1.EventEmitter {
      static create(fakeFs, path, opts) {
        let statWatcher = new _CustomStatWatcher(fakeFs, path, opts);
        return statWatcher.start(), statWatcher;
      }
      constructor(fakeFs, path, { bigint = !1 } = {}) {
        super(), this.status = Status.Ready, this.changeListeners = /* @__PURE__ */ new Map(), this.startTimeout = null, this.fakeFs = fakeFs, this.path = path, this.bigint = bigint, this.lastStats = this.stat();
      }
      start() {
...
var require_watchFile = __commonJS({
  "../../node_modules/@yarnpkg/fslib/lib/algorithms/watchFile.js"(exports) {
    ...
    function watchFile(fakeFs, path, a, b) {
      ...
      typeof a === "function" ? (bigint = !1, persistent = !0, interval = 5007, listener = a) : ({
        bigint = !1,
        persistent = !0,
        interval = 5007
      } = a, listener = b);
      ...
      return typeof statWatcher > "u" && (statWatcher = CustomStatWatcher_1.CustomStatWatcher.create(fakeFs, path, { bigint }), statWatchers.set(path, statWatcher)), statWatcher.registerChangeListener(listener, { persistent, interval }), statWatcher;
    }
    ...
    function unwatchFile(fakeFs, path, cb) {
      ...
      if (typeof statWatchers > "u")
        return;
      let statWatcher = statWatchers.get(path);
      typeof statWatcher > "u" || (typeof cb > "u" ? statWatcher.unregisterAllChangeListeners() : statWatcher.unregisterChangeListener(cb), statWatcher.hasChangeListeners() || (statWatcher.stop(), statWatchers.delete(path)));
    }
    exports.unwatchFile = unwatchFile;
    function unwatchAllFiles(fakeFs) {
      let statWatchers = statWatchersByFakeFS.get(fakeFs);
      if (!(typeof statWatchers > "u"))
        for (let path of statWatchers.keys())
  ...
  "../../node_modules/@yarnpkg/fslib/lib/ZipFS.js"(exports) {
    ...
    var ZipFS = class extends FakeFS_1.BasePortableFakeFS {
      constructor(source, opts) {
        ...
        for (let t4 = 0; t4 < entryCount; ++t4) {
          let raw = this.libzip.getName(this.zip, t4, 0);
          if (path_1.ppath.isAbsolute(raw))
            continue;
          let p = path_1.ppath.resolve(path_1.PortablePath.root, raw);
          this.registerEntry(p, t4), raw.endsWith("/") && this.registerListing(p);
        }
      ...
      makeLibzipError(error) {
        let errorCode2 = this.libzip.struct.errorCodeZip(error), strerror = this.libzip.error.strerror(error), libzipError = new errors.LibzipError(strerror, this.libzip.errors[errorCode2]);
        if (errorCode2 === this.libzip.errors.ZIP_ER_CHANGED)
      ...
      statImpl(reason, p, opts = {}) {
        ...
        if (typeof entry < "u") {
          let stat2 = this.libzip.struct.statS();
          if (this.libzip.statIndex(this.zip, entry, 0, 0, stat2) === -1)
            throw this.makeLibzipError(this.libzip.getError(this.zip));
          let uid = this.stats.uid, gid = this.stats.gid, size = this.libzip.struct.statSize(stat2) >>> 0, blksize = 512, blocks = Math.ceil(size / blksize), mtimeMs = (this.libzip.struct.statMtime(stat2) >>> 0) * 1e3, atimeMs = mtimeMs, birthtimeMs = mtimeMs, ctimeMs = mtimeMs, atime = new Date(atimeMs), birthtime = new Date(birthtimeMs), ctime = new Date(ctimeMs), mtime = new Date(mtimeMs), type = this.listings.has(p) ? constants_1.S_IFDIR : this.isSymbolicLink(entry) ? constants_1.S_IFLNK : constants_1.S_IFREG, defaultMode = type === constants_1.S_IFDIR ? 493 : 420, mode = type | this.getUnixMode(entry, defaultMode) & 511, crc = this.libzip.struct.statCrc(stat2), statInstance = Object.assign(new statUtils.StatEntry(), { uid, gid, size, blksize, blocks, atime, birthtime, ctime, mtime, atimeMs, birthtimeMs, ctimeMs, mtimeMs, mode, crc });
      ...
      getFileSource(index, opts = { asyncDecompress: !1 }) {
        ...
        let stat2 = this.libzip.struct.statS();
        if (this.libzip.statIndex(this.zip, index, 0, 0, stat2) === -1)
          throw this.makeLibzipError(this.libzip.getError(this.zip));
        let size = this.libzip.struct.statCompSize(stat2), compressionMethod = this.libzip.struct.statCompMethod(stat2), buffer = this.libzip.malloc(size);
        try {
      ...
      fdToPath(fd, reason) {
        var _a;
        let path = (_a = this.fds.get(fd)) === null || _a === void 0 ? void 0 : _a.p;
        if (typeof path > "u")
...
var require_CwdFS = __commonJS({
  "../../node_modules/@yarnpkg/fslib/lib/CwdFS.js"(exports) {
    ...
    var NodeFS_1 = require_NodeFS(), ProxiedFS_1 = require_ProxiedFS(), path_1 = require_path(), CwdFS = class extends ProxiedFS_1.ProxiedFS {
      ...
      mapToBase(path) {
        return this.pathUtils.isAbsolute(path) ? path : this.pathUtils.join(this.target, path);
      }
    };
    exports.CwdFS = CwdFS;
  }
...
var require_NoFS = __commonJS({
  "../../node_modules/@yarnpkg/fslib/lib/NoFS.js"(exports) {
    ...
    Object.defineProperty(exports, "__esModule", { value: !0 });
    exports.NoFS = void 0;
    var FakeFS_1 = require_FakeFS(), path_1 = require_path(), makeError = () => Object.assign(new Error("ENOSYS: unsupported filesystem access"), { code: "ENOSYS" }), NoFS = class extends FakeFS_1.FakeFS {
      constructor() {
        super(path_1.ppath);
      }
      getExtractHint() {
...
var require_ZipOpenFS = __commonJS({
  "../../node_modules/@yarnpkg/fslib/lib/ZipOpenFS.js"(exports) {
    ...
    var tslib_1 = (init_tslib_es6(), __toCommonJS(tslib_es6_exports)), fs_1 = __require("fs"), FakeFS_1 = require_FakeFS(), NodeFS_1 = require_NodeFS(), ZipFS_1 = require_ZipFS(), watchFile_1 = require_watchFile(), errors = tslib_1.__importStar(require_errors2()), path_1 = require_path(), ZIP_MASK = 4278190080, ZIP_MAGIC = 704643072, getArchivePart = (path, extension) => {
      let idx = path.indexOf(extension);
      if (idx <= 0)
        return null;
      let nextCharIdx = idx;
      for (; idx >= 0 && (nextCharIdx = idx + extension.length, path[nextCharIdx] !== path_1.ppath.sep); ) {
        if (path[idx - 1] === path_1.ppath.sep)
          return null;
        idx = path.indexOf(extension, nextCharIdx);
      }
      return path.length > nextCharIdx && path[nextCharIdx] !== path_1.ppath.sep ? null : path.slice(0, nextCharIdx);
    };
    ...
    var ZipOpenFS2 = class _ZipOpenFS extends FakeFS_1.BasePortableFakeFS {
      ...
      saveAndClose() {
        if ((0, watchFile_1.unwatchAllFiles)(this), this.zipInstances)
          for (let [path, { zipFs }] of this.zipInstances.entries())
            zipFs.saveAndClose(), this.zipInstances.delete(path);
      }
      discardAndClose() {
        if ((0, watchFile_1.unwatchAllFiles)(this), this.zipInstances)
          for (let [path, { zipFs }] of this.zipInstances.entries())
            zipFs.discardAndClose(), this.zipInstances.delete(path);
      }
      resolve(p) {
      ...
      limitOpenFiles(max) {
        ...
        for (let [path, { zipFs, expiresAt, refCount }] of this.zipInstances.entries())
          if (!(refCount !== 0 || zipFs.hasOpenFileHandles())) {
            if (now >= expiresAt) {
              zipFs.saveAndClose(), this.zipInstances.delete(path), closeCount -= 1;
              continue;
...
var require_lib = __commonJS({
  "../../node_modules/@yarnpkg/fslib/lib/index.js"(exports) {
    ...
    var tslib_1 = (init_tslib_es6(), __toCommonJS(tslib_es6_exports)), constants2 = tslib_1.__importStar(require_constants());
      ...
      return typeof __filename < "u" && (_scriptDir = _scriptDir || __filename), function(createModule2) {
        ...
        var PATH = {
          ...
          normalize: function(path) {
            var isAbsolute3 = path.charAt(0) === "/", trailingSlash = path.substr(-1) === "/";
            return path = PATH.normalizeArray(
              path.split("/").filter(function(p) {
                return !!p;
              }),
              !isAbsolute3
            ).join("/"), !path && !isAbsolute3 && (path = "."), path && trailingSlash && (path += "/"), (isAbsolute3 ? "/" : "") + path;
          },
...
`));
        ...
        var MEMFS = {
          ...
          getMode: function(path) {
            var stat2;
            try {
              stat2 = fs.lstatSync(path), NODEFS.isWindows && (stat2.mode = stat2.mode | (stat2.mode & 292) >> 2);
            } catch (e) {
          ...
          node_ops: {
            getattr: function(node) {
              var path = NODEFS.realPath(node), stat2;
              try {
                stat2 = fs.lstatSync(path);
              } catch (e) {
                throw e.code ? new FS.ErrnoError(NODEFS.convertNodeCode(e)) : e;
              }
              return NODEFS.isWindows && !stat2.blksize && (stat2.blksize = 4096), NODEFS.isWindows && !stat2.blocks && (stat2.blocks = (stat2.size + stat2.blksize - 1) / stat2.blksize | 0), {
                dev: stat2.dev,
                ino: stat2.ino,
                mode: stat2.mode,
                nlink: stat2.nlink,
            ...
            setattr: function(node, attr) {
              ...
              try {
                if (attr.mode !== void 0 && (fs.chmodSync(path, attr.mode), node.mode = attr.mode), attr.timestamp !== void 0) {
                  var date = new Date(attr.timestamp);
                  fs.utimesSync(path, date, date);
                }
                attr.size !== void 0 && fs.truncateSync(path, attr.size);
              } catch (e) {
                throw e.code ? new FS.ErrnoError(NODEFS.convertNodeCode(e)) : e;
              }
            },
            lookup: function(parent, name) {
              var path = PATH.join2(NODEFS.realPath(parent), name), mode = NODEFS.getMode(path);
              return NODEFS.createNode(parent, name, mode);
            },
            mknod: function(parent, name, mode, dev) {
              var node = NODEFS.createNode(parent, name, mode, dev), path = NODEFS.realPath(node);
              try {
                FS.isDir(node.mode) ? fs.mkdirSync(path, node.mode) : fs.writeFileSync(path, "", { mode: node.mode });
              } catch (e) {
                throw e.code ? new FS.ErrnoError(NODEFS.convertNodeCode(e)) : e;
              }
              return node;
            ...
            readlink: function(node) {
              var path = NODEFS.realPath(node);
              try {
                return path = fs.readlinkSync(path), path = NODEJS_PATH.relative(
          ...
          stream_ops: {
            open: function(stream) {
              ...
              try {
                FS.isFile(stream.node.mode) && (stream.nfd = fs.openSync(path, NODEFS.flagsForNode(stream.flags)));
              } catch (e) {
                throw e.code ? new FS.ErrnoError(NODEFS.convertNodeCode(e)) : e;
              }
            },
          ...
          mknod: function(path, mode) {
            FS.isDir(path) ? fs.mkdirSync(path, mode) : fs.writeFileSync(path, "", { mode });
          },
          mkdir: function() {
          ...
          open: function(path, flags, mode, suggestFD) {
            ...
            var nfd = fs.openSync(path, NODEFS.flagsForNode(flags), mode), fd = suggestFD ?? FS.nextfd(nfd), stream = {
              fd,
              nfd,
              position: 0,
              path,
          ...
          lookupPath: function(path, opts) {
            if (path = PATH_FS.resolve(FS.cwd(), path), opts = opts || {}, !path) return { path: "", node: null };
          ...
          rename: function(old_path, new_path) {
            ...
            var old_node = FS.lookupNode(old_dir, old_name), relative6 = PATH_FS.relative(old_path, new_dirname);
            if (relative6.charAt(0) !== ".")
              throw new FS.ErrnoError(28);
            if (relative6 = PATH_FS.relative(new_path, old_dirname), relative6.charAt(0) !== ".")
              throw new FS.ErrnoError(55);
            var new_node;
            try {
              new_node = FS.lookupNode(new_dir, new_name);
          ...
          unlink: function(path) {
            var lookup = FS.lookupPath(path, { parent: !0 }), parent = lookup.node, name = PATH.basename(path), node = FS.lookupNode(parent, name), errCode = FS.mayDelete(parent, name, !1);
            if (errCode)
              throw new FS.ErrnoError(errCode);
          ...
          open: function(path, flags, mode, fd_start, fd_end) {
            ...
            stream.stream_ops.open && stream.stream_ops.open(stream), Module.logReadFiles && !(flags & 1) && (FS.readFiles || (FS.readFiles = {}), path in FS.readFiles || (FS.readFiles[path] = 1, err("FS.trackingDelegate error on read file: " + path)));
            try {
              if (FS.trackingDelegate.onOpenFile) {
                var trackingFlags = 0;
                (flags & 2097155) !== 1 && (trackingFlags |= FS.tracking.openFlags.READ), (flags & 2097155) !== 0 && (trackingFlags |= FS.tracking.openFlags.WRITE), FS.trackingDelegate.onOpenFile(path, trackingFlags);
              }
            } catch (e) {
              err(
          ...
          readFile: function(path, opts) {
            ...
            var ret, stream = FS.open(path, opts.flags), stat2 = FS.stat(path), length = stat2.size, buf = new Uint8Array(length);
          ...
          createDataFile: function(parent, name, data, canRead, canWrite, canOwn) {
            var path = name ? PATH.join2(
              typeof parent == "string" ? parent : FS.getPath(parent),
              name
            ) : parent, mode = FS.getMode(canRead, canWrite), node = FS.create(path, mode);
            if (data) {
              if (typeof data == "string") {
            ...
            openRequest.onupgradeneeded = onerror, openRequest.onsuccess = function() {
              ...
              paths.forEach(function(path) {
                ...
                getRequest.onsuccess = function() {
                  FS.analyzePath(path).exists && FS.unlink(path), FS.createDataFile(
                    PATH.dirname(path),
                    PATH.basename(path),
                    getRequest.result,
                    !0,
          ...
          doStat: function(func, path, buf) {
            ...
            return HEAP32[buf >> 2] = stat2.dev, HEAP32[buf + 4 >> 2] = 0, HEAP32[buf + 8 >> 2] = stat2.ino, HEAP32[buf + 12 >> 2] = stat2.mode, HEAP32[buf + 16 >> 2] = stat2.nlink, HEAP32[buf + 20 >> 2] = stat2.uid, HEAP32[buf + 24 >> 2] = stat2.gid, HEAP32[buf + 28 >> 2] = stat2.rdev, HEAP32[buf + 32 >> 2] = 0, tempI64 = [
              stat2.size >>> 0,
              (tempDouble = stat2.size, +Math.abs(tempDouble) >= 1 ? tempDouble > 0 ? (Math.min(+Math.floor(tempDouble / 4294967296), 4294967295) | 0) >>> 0 : ~~+Math.ceil(
                (tempDouble - +(~~tempDouble >>> 0)) / 4294967296
              ) >>> 0 : 0)
            ], HEAP32[buf + 40 >> 2] = tempI64[0], HEAP32[buf + 44 >> 2] = tempI64[1], HEAP32[buf + 48 >> 2] = 4096, HEAP32[buf + 52 >> 2] = stat2.blocks, HEAP32[buf + 56 >> 2] = stat2.atime.getTime() / 1e3 | 0, HEAP32[buf + 60 >> 2] = 0, HEAP32[buf + 64 >> 2] = stat2.mtime.getTime() / 1e3 | 0, HEAP32[buf + 68 >> 2] = 0, HEAP32[buf + 72 >> 2] = stat2.ctime.getTime() / 1e3 | 0, HEAP32[buf + 76 >> 2] = 0, tempI64 = [
              stat2.ino >>> 0,
              (tempDouble = stat2.ino, +Math.abs(tempDouble) >= 1 ? tempDouble > 0 ? (Math.min(+Math.floor(tempDouble / 4294967296), 4294967295) | 0) >>> 0 : ~~+Math.ceil(
                (tempDouble - +(~~tempDouble >>> 0)) / 4294967296
              ) >>> 0 : 0)
          ...
          doMkdir: function(path, mode) {
            return path = PATH.normalize(path), path[path.length - 1] === "/" && (path = path.substr(0, path.length - 1)), FS.mkdir(path, mode, 0), 0;
          },
          ...
          doAccess: function(path, amode) {
            if (amode & -8)
              return -28;
            var node, lookup = FS.lookupPath(path, { follow: !0 });
        ...
        function ___sys_open(path, flags, varargs) {
          SYSCALLS.varargs = varargs;
          try {
            var pathname = SYSCALLS.getStr(path), mode = varargs ? SYSCALLS.get() : 0, stream = FS.open(pathname, flags, mode);
            return stream.fd;
          } catch (e) {
            return (typeof FS > "u" || !(e instanceof FS.ErrnoError)) && abort(e), -e.errno;
          }
...
var typescriptFallbackExtensions = {
  ".js": [".ts", ".tsx"],
  ".mjs": [".mts"],
  ".cjs": [".cts"],
  ".jsx": [".tsx"]
}, supportedExtensions = storybookConfigExtensions;
...
var importResolver = createModuleResolver({
...
function resolveImport(id, options) {
  try {
    ...
    let ext = extname(id), fallbackExtensions = typescriptFallbackExtensions[ext];
    if (!fallbackExtensions)
      throw error;
    let fallbackError = error, baseId = id.slice(0, -ext.length);
    for (let fallbackExtension of fallbackExtensions)
      try {
...
function resolveSync(id, basedir) {
...
async function validateConfigurationFiles(configDir, cwd) {
  let extensionsPattern = `{${Array.from(supportedExtensions).join(",")}}`, mainConfigMatches = await glob(slash(resolve2(configDir, `main${extensionsPattern}`)), {
    cwd: cwd ?? process.cwd()
...
function readAddonManifest(dir) {
  try {
    let { name, exports } = JSON.parse(readFileSync(join(dir, "package.json"), "utf8"));
    return { name: typeof name == "string" && name !== "" ? name : void 0, exports };
  } catch {
    return;
  }
}
var resolveAddonName = (configDir, name, options) => {
  ...
  let manifest = isAbsolute(name) ? readAddonManifest(name) : void 0, exportsSpecifier = manifest?.exports ? manifest.name : void 0, resolveEntryFile = (entry) => (exportsSpecifier ? safeResolveModule({ specifier: `${exportsSpecifier}/${entry}`, parent: name }) : void 0) ?? safeResolveModule({ specifier: join(name, entry), parent: configDir }), presetFile = resolveEntryFile("preset"), managerFile = resolveEntryFile("manager"), previewFile = resolveEntryFile("preview");
...
${item}`
  ...
  if (!resolved) {
    let packageName = isAbsolute(name) ? readAddonManifest(name)?.name : void 0;
    logger2.warn(
      packageName ? import_ts_dedent2.dedent`
...
import { mkdirSync, readFileSync as readFileSync2, readdirSync, rmSync, writeFileSync } from "node:fs";
...
var FileSystemCache = class {
  ...
  getSync(name, fallback) {
    try {
      let data = readFileSync2(this.generateHash(name), "utf8");
      return this.parseCacheData(data, fallback);
    } catch {
      return fallback;
    }
...
// src/common/utils/find-storybook-package-root.ts
import { readFileSync as readFileSync3, realpathSync } from "node:fs";
import { fileURLToPath } from "node:url";
var ownDir = normalize(dirname(fileURLToPath(import.meta.url)));
function findStorybookPackageRoot() {
  let dir = ownDir;
  for (; ; ) {
...
function readManifestName(dir) {
  try {
    let manifest = JSON.parse(readFileSync3(join(dir, "package.json"), "utf-8"));
...
import {
  CoreWebpackCompiler,
  SupportedBuilder as SupportedBuilder2,
  SupportedFramework as SupportedFramework2,
  SupportedRenderer as SupportedRenderer2
} from "storybook/internal/types";

// src/shared/constants/config-folder.ts
...
import { dirname as dirname3, isAbsolute as isAbsolute2, join as join6, normalize as normalize2, resolve as resolve3 } from "node:path";
...
var JsPackageManager = class _JsPackageManager {
  ...
  constructor(options) {
    this.cwd = options?.cwd || process.cwd(), this.instanceDir = options?.configDir ? isAbsolute2(options?.configDir) ? dirname3(options?.configDir) : dirname3(join6(this.cwd, options?.configDir)) : this.cwd, this.packageJsonPaths = _JsPackageManager.listAllPackageJsonPaths(
      this.instanceDir,
  ...
  isStorybookInMonorepo() {
    ...
    for (let packageJsonPath of packageJsonPaths) {
      let packageJsonFile = readFileSync4(packageJsonPath, "utf8");
  ...
  static getPackageJson(packageJsonPath) {
    let absolutePath = normalize2(resolve3(packageJsonPath)), cached = _JsPackageManager.packageJsonCache.get(absolutePath);
    if (cached)
      return logger5.debug(`Using cached package.json for ${absolutePath}...`), cached;
    let jsonContent = readFileSync4(absolutePath, "utf8"), packageJSON = JSON.parse(jsonContent);
  ...
  writePackageJson(packageJson, directory = this.cwd) {
    ...
    let filePath = join6(directory, "package.json"), indent = this.#getIndent(filePath), packageJsonPath = normalize2(resolve3(directory, "package.json")), content = `${JSON.stringify(packageJsonToWrite, null, indent)}
`;
   ...
   */
  async getDeclaredVersionSpecifier(packageName) {
    let installed = await this.getInstalledVersion(packageName);
    if (installed)
      return installed;
    let declared = this.getAllDependencies()[packageName];
   ...
   */
  applyVersionToRelatedPackages(packages, version, _anchorPackage) {
  ...
  async addDependencies(options, dependencies) {
    ...
    if (skipInstall) {
      ...
      for (let dep of dependencies) {
        let [packageName, packageVersion] = getPackageDetails(dep);
      ...
      try {
        ...
        throw logger5.error(`
An error occurred while adding dependencies to your package.json:`), logger5.log(String(e)), e?.fromStorybook ? e : new HandledError(e);
      }
  }
  /**
  ...
  async latestVersion(packageName, constraint) {
    ...
    try {
      ...
      return logger5.debug(`Failed to fetch the latest version for ${packageName}: ${String(e)}`), _JsPackageManager.latestVersionCache.set(cacheKey, null), null;
    }
  }
  /**
  ...
  getDependencyVersion(dependency) {
    return logger5.debug(`Getting dependency version for ${dependency}...`), this.packageJsonPaths.map((path) => {
      let packageJson = _JsPackageManager.getPackageJson(path);
  ...
  static hasStorybookDependency(packageJsonPath) {
    try {
      let content = readFileSync4(packageJsonPath, "utf-8"), packageJson = JSON.parse(content);
  ...
  static hasAnyStorybookDependency(packageJsonPath) {
    try {
      let content = readFileSync4(packageJsonPath, "utf-8"), packageJson = JSON.parse(content), allDeps = {
...
}, getAddonNames = (mainConfig) => (mainConfig.addons || []).map(normalizeAddonName).filter((item) => item != null);
...
import { join as join7, parse as parse2, relative, resolve as resolve4 } from "node:path";
...
async function loadMainConfig({
  ...
  let mainPath = getInterpretedFile(resolve4(configDir, "main"));
  try {
...
var rendererPackages = {
  ...
  "@storybook/angular-vite": SupportedFramework2.ANGULAR_VITE,
  ...
  for (let packageJsonPath of packageJsonPaths) {
    let packageJson = JSON.parse(readFileSync5(packageJsonPath, "utf-8")), [dep, devDep, peerDep] = findDependency(packageJson, ([key]) => key === "storybook"), [pkg, version] = dep || devDep || peerDep || [];
    if (pkg && version)
      return version;
  }
  ...
  if (!existsSync3(storybookConfigDir)) {
    ...
    for (let packageJsonPath of packageJsonPaths) {
      let storybookScript = JSON.parse(readFileSync5(packageJsonPath, "utf-8")).scripts?.storybook;
...
}, getStorybookInfo = async (configDir = ".storybook", cwd, { skipCache } = {}) => {
  ...
  if (!frameworkField)
    return basename(configInfo.configDir) === RN_STORYBOOK_DIR ? {
      ...configInfo,
      versionSpecifier,
      addons,
      mainConfig,
      ...
      renderer: SupportedRenderer2.REACT_NATIVE,
...
var extractFrameworkPackageName = (framework) => {
  ...
  return Object.keys(frameworkPackages).find(
    (pkg) => normalizedPath.endsWith(pkg) || // pnpm virtual-store dirs encode the scope slash as '+' and append '@<version>',
...
// src/common/utils/read-dependency-manifest.ts
import { readFile as readFile4 } from "node:fs/promises";
import { dirname as dirname5, join as join9 } from "node:path";
import { logger as logger7 } from "storybook/internal/node-logger";
var readDependencyManifest = async (directory, dependency) => {
  let subpath = from(directory, join9(dependency, "package.json"), !0);
  if (subpath)
    try {
      let manifest = JSON.parse(await readFile4(subpath, { encoding: "utf8" }));
      if (manifest?.name)
        return manifest;
    } catch {
    }
  let entry = from(directory, dependency, !0);
  if (!entry) {
    logger7.debug(`readDependencyManifest: could not resolve "${dependency}"`);
    return;
  }
  let dir = dirname5(entry);
  for (; ; ) {
    try {
      let manifest = JSON.parse(await readFile4(join9(dir, "package.json"), { encoding: "utf8" }));
      if (manifest?.name)
        return manifest;
    } catch {
    }
...
var getAutoRefs = async (options) => {
  ...
  if (!location)
    return {};
  let directory = dirname6(location), { dependencies = [], devDependencies = [] } = JSON.parse(await readFile5(location, { encoding: "utf8" })) || {}, deps = Object.keys({ ...dependencies, ...devDependencies });
  return (await Promise.all(
    deps.map(async (d) => {
      let manifest = await readDependencyManifest(directory, d);
...
function loadManagerOrAddonsFile({ configDir }) {
  let storybookCustomAddonsPath = getInterpretedFile(resolve5(configDir, "addons")), storybookCustomManagerPath = getInterpretedFile(resolve5(configDir, "manager"));
...
function loadPreviewOrConfigFile({ configDir }) {
  let storybookConfigPath = getInterpretedFile(resolve6(configDir, "config")), storybookPreviewPath = getInterpretedFile(resolve6(configDir, "preview"));
...
import { basename as basename2, dirname as dirname7, relative as relative2, resolve as resolve7 } from "node:path";
...
var DEFAULT_TITLE_PREFIX = "", DEFAULT_FILES_PATTERN = "**/*.@(mdx|stories.@(js|jsx|mjs|ts|tsx))", isDirectory = (configDir, entry) => {
  ...
  let directoryFromConfig = resolve7(configDir, directory), directoryFromWorking = relative2(workingDir, directoryFromConfig);
...
async function readTemplate(filename) {
  return readFile6(filename, {
    encoding: "utf8"
  });
}

// src/common/utils/resolve-meta-component.ts
import { types as t } from "storybook/internal/babel";
var unwrapComponentExpression = (node) => {
  let unwrapped = unwrapExpression(node);
  return t.isTSInstantiationExpression(unwrapped) ? unwrapComponentExpression(unwrapped.expression) : unwrapped;
};
function createMetaComponentResolver(options = {}) {
  let resolver = createModuleResolver({
    extensions: [...options.extensions ?? [], ...jsTsSourceExtensions],
    mainFields: ["module", "main"],
    tsconfig: "auto"
  }), resolveFile = (fromFile, specifier) => {
    try {
    ...
    let filePath = resolveFile(fromFile, specifier);
    return filePath ? parseReferenceModule(filePath) : void 0;
  }, fromIdentifier = (module, localName) => {
    let binding = findImport(module.program, localName);
    ...
    let { importId, exportName } = binding;
    return {
      component: { localName, importId, exportName, path: resolveFile(module.filePath, importId) }
    };
  };
  return function(csf, storyPath) {
    let node = csf._metaAnnotations.component;
    if (!node)
      return { reason: "no-meta-component" };
    let storyModule = { program: csf._file.path, filePath: storyPath }, expression = unwrapComponentExpression(node);
    if (t.isIdentifier(expression))
      return fromIdentifier(storyModule, expression.name);
    if (t.isMemberExpression(expression) && !expression.computed && t.isIdentifier(expression.object) && t.isIdentifier(expression.property)) {
      let namespace = findImport(csf._file.path, expression.object.name);
      if (namespace.kind === "namespace") {
        let exportName = expression.property.name;
        return {
          component: {
    ...
    let referenced = resolveReferencedValue(
      {
        ...storyModule,
        resolveModule,
        externalize: (n) => isSelfContained(n) ? n : void 0
      },
      expression
    );
    if (referenced) {
      let value = unwrapComponentExpression(referenced.node);
...
function findImport(program, localName) {
  for (let statement of program.get("body"))
    if (statement.isImportDeclaration())
      for (let specifier of statement.node.specifiers) {
        if (specifier.local.name !== localName)
          continue;
        if (statement.node.importKind === "type")
          return { kind: "unsupported" };
        let importId = statement.node.source.value;
        return t.isImportNamespaceSpecifier(specifier) ? { kind: "namespace", importId } : t.isImportDefaultSpecifier(specifier) ? { kind: "import", importId, exportName: "default" } : specifier.importKind === "type" ? { kind: "unsupported" } : {
          kind: "import",
          importId,
          exportName: t.isIdentifier(specifier.imported) ? specifier.imported.name : specifier.imported.value
        };
      }
  return { kind: "local" };
}
...
function getPreviewBodyTemplate(configDirPath, interpolations) {
  ...
  return existsSync4(bodyHtmlPath) && (result = readFileSync6(bodyHtmlPath, "utf8") + result), interpolate2(result, interpolations);
}
function getPreviewHeadTemplate(configDirPath, interpolations) {
  ...
  return existsSync4(headHtmlPath) && (result += readFileSync6(headHtmlPath, "utf8")), interpolate2(result, interpolations);
}

// src/common/utils/tsconfig.ts
import { existsSync as existsSync5, readFileSync as readFileSync7, statSync } from "node:fs";
import { basename as basename3, dirname as dirname8, extname as extname2, relative as relative3, resolve as resolve8 } from "node:path";
var import_picomatch = __toESM(require_picomatch(), 1);
var TSCONFIG_CANDIDATES = ["tsconfig.json", "tsconfig.base.json", "tsconfig.app.json"], DEFAULT_EXCLUDE_PATTERNS = ["node_modules", "bower_components", "jspm_packages"], findTsconfigPath = (cwd) => {
  let projectRoot = getProjectRoot();
  for (let candidate of TSCONFIG_CANDIDATES) {
    let found = up2(candidate, { cwd, last: projectRoot });
    if (found)
      return found;
  }
}, findTsconfigPathForFile = (cwd, filePath) => {
  let rootTsconfigPath = findTsconfigPath(cwd);
  if (!rootTsconfigPath)
    return;
  let absoluteFilePath = resolve8(filePath), matchingConfigs = collectTsconfigEntries(rootTsconfigPath, /* @__PURE__ */ new Set()).filter(
    (entry) => tsconfigIncludesFile(entry, absoluteFilePath)
  );
  return matchingConfigs.sort((left, right) => compareTsconfigEntries(absoluteFilePath, left, right)), matchingConfigs[0]?.path ?? rootTsconfigPath;
}, findTsconfigPathForPath = (path) => isDirectoryPath(path) ? findTsconfigPath(path) : findTsconfigPathForFile(dirname8(path), path), getTsconfigPathsBaseDir = (configPath) => {
  let normalizedConfigPath = resolve8(configPath), resolved = resolvePathsBase(normalizedConfigPath, /* @__PURE__ */ new Set());
  return resolved.absoluteBaseUrl ?? resolved.pathsBasePath ?? dirname8(normalizedConfigPath);
};
function resolvePathsBase(configPath, seen) {
  let normalizedConfigPath = resolve8(configPath);
  if (seen.has(normalizedConfigPath))
    return {};
  seen.add(normalizedConfigPath);
  let config = readTsconfigConfig(normalizedConfigPath);
  if (!config)
    return {};
  let inherited = {};
  for (let extendsPath of getExtendsPaths(normalizedConfigPath, config)) {
    let base = resolvePathsBase(extendsPath, seen);
    inherited = {
      absoluteBaseUrl: base.absoluteBaseUrl ?? inherited.absoluteBaseUrl,
      pathsBasePath: base.pathsBasePath ?? inherited.pathsBasePath
    };
  }
  let ownBaseUrl = config.compilerOptions?.baseUrl, ownPaths = config.compilerOptions?.paths;
  return {
    absoluteBaseUrl: typeof ownBaseUrl == "string" && ownBaseUrl.length > 0 ? resolve8(dirname8(normalizedConfigPath), ownBaseUrl) : inherited.absoluteBaseUrl,
    pathsBasePath: ownPaths != null && typeof ownPaths == "object" ? dirname8(normalizedConfigPath) : inherited.pathsBasePath
  };
}
...
function collectTsconfigEntries(configPath, seen) {
  let normalizedConfigPath = resolve8(configPath);
  if (seen.has(normalizedConfigPath))
    return [];
  seen.add(normalizedConfigPath);
  ...
  return config ? [
    ...
    ...getReferencedTsconfigPaths(normalizedConfigPath, config).flatMap(
      (referencePath) => collectTsconfigEntries(referencePath, seen)
    )
  ] : [];
}
function getReferencedTsconfigPaths(configPath, config) {
  return (Array.isArray(config.references) ? config.references : []).map((reference) => reference.path).filter((referencePath) => typeof referencePath == "string").map((referencePath) => resolve8(dirname8(configPath), referencePath)).flatMap((referencePath) => resolveReferenceConfigPaths(referencePath));
}
function resolveReferenceConfigPaths(referencePath) {
  return referencePath.endsWith(".json") ? [referencePath] : TSCONFIG_CANDIDATES.map((candidate) => resolve8(referencePath, candidate));
}
function tsconfigIncludesFile(entry, filePath) {
  let configDir = dirname8(entry.path), relativeFilePath = normalizePath3(relative3(configDir, filePath));
  if (relativeFilePath.startsWith("../") || relativeFilePath === ".." || isReferencesShell(entry.config))
    return !1;
  let ownership = resolveOwnershipSpec(entry.path, entry.config, /* @__PURE__ */ new Set());
  if (ownership.files && ownership.files.length > 0) {
    let normalizedFilePath = normalizePath3(filePath);
    return ownership.files.some(
      (candidatePath) => normalizePath3(resolve8(configDir, candidatePath)) === normalizedFilePath
    );
  }
  let includePatterns = getIncludePatterns(ownership, entry.config);
  ...
  return matchesPatterns(relativeFilePath, excludePatterns) ? !1 : matchesPatterns(relativeFilePath, includePatterns);
}
function resolveOwnershipSpec(configPath, config, seen) {
  let normalizedConfigPath = resolve8(configPath);
  if (seen.has(normalizedConfigPath))
    return {};
  seen.add(normalizedConfigPath);
  let ownership = {};
  for (let extendsPath of getExtendsPaths(normalizedConfigPath, config)) {
    let baseConfig = readTsconfigConfig(extendsPath);
    if (!baseConfig)
      continue;
    let baseOwnership = resolveOwnershipSpec(extendsPath, baseConfig, seen);
    ownership = mergeOwnershipSpec(
      ownership,
      rebaseOwnershipSpec(baseOwnership, dirname8(extendsPath), dirname8(normalizedConfigPath))
    );
  }
  return mergeOwnershipSpec(ownership, ownOwnershipSpec(config));
}
function getExtendsPaths(configPath, config) {
  return (typeof config.extends == "string" ? [config.extends] : Array.isArray(config.extends) ? config.extends.filter((value) => typeof value == "string") : []).map((extendsPath) => resolveExtendsPath(configPath, extendsPath));
}
function resolveExtendsPath(configPath, extendsPath) {
  let resolved = resolve8(dirname8(configPath), extendsPath);
  if (existsSync5(resolved))
    return resolved;
  if (!resolved.endsWith(".json")) {
    let withJson = `${resolved}.json`;
    if (existsSync5(withJson))
      return withJson;
  }
  return resolved;
}
function ownOwnershipSpec(config) {
  let ownership = {};
  return config.files !== void 0 && (ownership.files = asStringArray(config.files).map(normalizePath3)), config.include !== void 0 && (ownership.include = asStringArray(config.include).map(normalizePath3)), config.exclude !== void 0 && (ownership.exclude = asStringArray(config.exclude).map(normalizePath3)), ownership;
}
function mergeOwnershipSpec(base, overlay) {
  return {
    files: overlay.files !== void 0 ? overlay.files : base.files,
    include: overlay.include !== void 0 ? overlay.include : base.include,
    exclude: overlay.exclude !== void 0 ? overlay.exclude : base.exclude
  };
}
function rebaseOwnershipSpec(ownership, fromDir, toDir) {
  return {
...
function rebasePattern(pattern, fromDir, toDir) {
  return normalizePath3(relative3(toDir, resolve8(fromDir, pattern)));
}
function isReferencesShell(config) {
  if (!Array.isArray(config.references))
    return !1;
  let hasOwnFiles = Array.isArray(config.files) && config.files.length > 0, hasOwnInclude = Array.isArray(config.include) && config.include.length > 0;
  return !hasOwnFiles && !hasOwnInclude;
}
function getIncludePatterns(ownership, leafConfig) {
  return ownership.include && ownership.include.length > 0 ? ownership.include.map(normalizeTsconfigPattern) : ownership.files && ownership.files.length > 0 ? ownership.files.map(normalizePath3) : Array.isArray(leafConfig.files) && leafConfig.files.length === 0 && !ownership.include ? [] : ["**/*"];
}
function compareTsconfigEntries(filePath, left, right) {
  let leftDir = dirname8(left.path), rightDir = dirname8(right.path), leftDepth = normalizePath3(relative3(leftDir, filePath)).split("/").length, rightDepth = normalizePath3(relative3(rightDir, filePath)).split("/").length;
  if (leftDepth !== rightDepth)
    return leftDepth - rightDepth;
  let leftIsPrimaryTsconfig = basename3(left.path) === "tsconfig.json" ? 1 : 0;
  return (basename3(right.path) === "tsconfig.json" ? 1 : 0) - leftIsPrimaryTsconfig;
}
function readTsconfigConfig(configPath) {
  try {
    let content = readFileSync7(configPath, "utf-8");
    return JSON.parse(stripJsonComments(content, { trailingCommas: !0 }));
  } catch {
    return;
  }
}
function matchesPatterns(filePath, patterns) {
  return patterns.some((pattern) => (0, import_picomatch.default)(pattern, { dot: !0 })(filePath));
}
function normalizeTsconfigPattern(pattern) {
  let normalizedPattern = normalizePath3(pattern);
  return normalizedPattern.endsWith("/") ? `${normalizedPattern}**/*` : !import_picomatch.default.scan(normalizedPattern).isGlob && !/\.[^/]+$/.test(normalizedPattern) ? `${normalizedPattern}/**/*` : normalizedPattern;
}
...
// src/common/utils/babel.ts
import { readFileSync as readFileSync8 } from "node:fs";
import { createRequire } from "node:module";
import { logger as logger10 } from "storybook/internal/node-logger";
var require2 = createRequire(import.meta.url), getBabelPresetEnvMajor = () => {
  try {
    let pkgPath = require2.resolve("@babel/preset-env/package.json"), { version } = JSON.parse(readFileSync8(pkgPath, "utf8")), parsed = Number.parseInt(version, 10);
...
import { sanitize as sanitize2, storyNameFromExport, toId } from "storybook/internal/csf/csf-utils";
...
function getStoryTitle({
  ...
  let normalizedStories = normalizeStories2(stories, {
    configDir,
    workingDir
  }), relativePath = relative4(workingDir, storyFilePath), importPath = posix3(normalizeStoryPath2(relativePath));
...
// src/common/utils/lazy-docgen-middleware.ts
function createLazyDocgenMiddleware({
  createManager,
  extract
}) {
  let managerPromise, getManager = () => managerPromise ??= createManager();
  return (nextDocgen) => async (input) => {
    let storyImportPath = getStoryImportPathFromEntry(input.entry);
    if (!storyImportPath || !STORY_FILE_TEST_REGEXP.test(storyImportPath))
      return nextDocgen(input);
    let manager = await getManager();
    if (!manager)
      return nextDocgen(input);
    let ours = await extract(manager, input);
...
async function syncPreviewAddonsWithMainConfig(mainConfig, previewConfig, configDir) {
  ...
  existingAddons || previewConfig.setFieldNode(["addons"], t2.arrayExpression([]));
  ...
  for (let addon of addons) {
    ...
    if (annotations) {
      if (previewConfig._ast.program.body.find(
        (node) => t2.isImportDeclaration(node) && node.source.value === annotations.importPath
      ))
        continue;
      (!existingAddons || t2.isArrayExpression(existingAddons) && !existingAddons.elements.some(
        (element) => t2.isIdentifier(element) && element.name === annotations.importName
        ...
        ["addons"],
        t2.callExpression(t2.identifier(annotations.importName), [])
      )) : (previewConfig.setImport({ namespace: annotations.importName }, annotations.importPath), previewConfig.appendNodeToArray(["addons"], t2.identifier(annotations.importName))));
    }
  }
  return syncedAddons.length > 0 && logger11.log(
...
function doesVariableOrFunctionDeclarationExist(node, name) {
  return t3.isVariableDeclaration(node) && node.declarations.length === 1 && t3.isVariableDeclarator(node.declarations[0]) && t3.isIdentifier(node.declarations[0].id) && node.declarations[0].id?.name === name || t3.isFunctionDeclaration(node) && t3.isIdentifier(node.id) && node.id.name === name;
}
function getReferenceToGetAbsolutePathWrapper(config, value) {
  return t3.callExpression(
    t3.identifier(getAbsolutePathWrapperName(config) ?? PREFERRED_GET_ABSOLUTE_PATH_WRAPPER_NAME),
...
function isGetAbsolutePathWrapperNecessary(node, cb = () => {
  ...
  if (t3.isObjectExpression(node)) {
    let nameProperty = node.properties.find(
      (property) => t3.isObjectProperty(property) && t3.isIdentifier(property.key) && property.key.name === "name"
    );
    if (nameProperty && t3.isStringLiteral(nameProperty.value))
      return cb(nameProperty), !0;
  }
  return t3.isArrayExpression(node) && node.elements.some((element) => element && isGetAbsolutePathWrapperNecessary(element)) ? (cb(node), !0) : !1;
}
function getFieldsForGetAbsolutePathWrapper(config) {
  ...
  return [
    ...
    ...addons && t3.isArrayExpression(addons) ? [addons] : []
  ];
}
function getAbsolutePathWrapperAsCallExpression(isConfigTypescript) {
  let functionDeclaration = {
    ...t3.functionDeclaration(
      t3.identifier(PREFERRED_GET_ABSOLUTE_PATH_WRAPPER_NAME),
      [
        {
          ...t3.identifier("value"),
          ...isConfigTypescript ? { typeAnnotation: t3.tsTypeAnnotation(t3.tSStringKeyword()) } : {}
        }
      ],
      t3.blockStatement([
        t3.returnStatement(
          t3.callExpression(t3.identifier("dirname"), [
            t3.callExpression(t3.identifier("fileURLToPath"), [
              t3.callExpression(
                t3.memberExpression(
                  t3.metaProperty(t3.identifier("import"), t3.identifier("meta")),
                  t3.identifier("resolve")
                ),
                [
                  t3.templateLiteral(
                    [
                      t3.templateElement({ raw: "" }),
                      t3.templateElement({ raw: "/package.json" }, !0)
                    ],
                    [t3.identifier("value")]
                  )
                ]
    ...
    ...isConfigTypescript ? { returnType: t3.tSTypeAnnotation(t3.tsAnyKeyword()) } : {}
  };
...
function wrapValueWithGetAbsolutePathWrapper(config, node) {
  isGetAbsolutePathWrapperNecessary(node, (n) => {
    ...
    t3.isObjectProperty(n) && t3.isStringLiteral(n.value) && (n.value = getReferenceToGetAbsolutePathWrapper(config, n.value.value)), t3.isArrayExpression(n) && n.elements.forEach((element) => {
...
import { basename as basename4, join as join16, parse as parse3, relative as relative5 } from "node:path";

// src/common/utils/command.ts
import { logger as logger12, prompt as prompt2 } from "storybook/internal/node-logger";
...
function getRemotePackageRunnerArgs(packageManagerType, pkg, version, args) {
  let pkgWithVersion = `${pkg}@${version}`;
  return packageManagerType === "npm" /* NPM */ || packageManagerType === "yarn1" /* YARN1 */ ? ["--yes", pkgWithVersion, ...args] : [pkgWithVersion, ...args];
}
function getVitestStorybookRunCommand(packageManager, file) {
  let args = ["vitest", "--project", "storybook", "run"];
  return file && args.push(file), packageManager.getPackageCommand(args);
}
function getMswInitCommand(packageManager) {
  return packageManager.getPackageCommand(["msw", "init", "./public", "--save"]);
}
var getStorybookRerunCommand = (installContext, compatibleVersion) => installContext === "create" ? compatibleVersion ? `npx create-storybook@${compatibleVersion}` : "npx create-storybook@<compatible-version>" : compatibleVersion ? `npx storybook@${compatibleVersion} upgrade` : "npx storybook@<compatible-version> upgrade", getStorybookRerunInstruction = (installContext) => installContext === "create" ? "Please rerun Storybook creation with:" : "Please rerun the Storybook upgrade with:", getErrorLogs = (error) => {
...
// src/common/utils/categorize-execa-error.ts
var PACKAGE_MANAGER_COMMANDS = /* @__PURE__ */ new Set(["npm", "pnpm", "yarn", "bun"]), PACKAGE_INSTALL_ARGS = /* @__PURE__ */ new Set(["install", "add", "ci", "i"]);
function extractExecaCommandErrorData(error, context) {
  let structured = error, args = context.args ?? [], logs = getErrorLogs(error);
  return logs === "[object Object]" && (logs = ""), {
...
function extractPackageManagerErrorCode(logs) {
  let npmMatch = logs.match(/npm\s+(?:ERR!|error)\s+code\s+([A-Z0-9_]+)/i);
  if (npmMatch)
    return npmMatch[1];
  let pnpmMatch = logs.match(/(ERR_PNPM_[A-Z0-9_]+)/);
  if (pnpmMatch)
    return pnpmMatch[1];
}
function isAutomigrateAddonA11yTest(args) {
  let joined = args.join(" ");
  return joined.includes("automigrate") && joined.includes("addon-a11y-addon-test");
}
function isNuxtModuleAdd(args) {
  let joined = args.join(" ");
  return joined.includes("nuxi") && joined.includes("module add") && joined.includes("storybook");
}
function isPlaywrightInstall(args) {
  let joined = args.join(" ");
  return joined.includes("playwright") && joined.includes("install");
}
function isPackageManagerCommand(command) {
  let normalizedCommand = command.replace(/\.(cmd|exe|ps1)$/i, "");
  return PACKAGE_MANAGER_COMMANDS.has(normalizedCommand);
}
function isPackageInstall(command, args) {
  return isPackageManagerCommand(command) ? args.some((arg) => PACKAGE_INSTALL_ARGS.has(arg)) : !1;
}
function isBinaryNotFound(data) {
  let logs = data.logs.toLowerCase();
  return data.exitCode === 127 || logs.includes("enoent") && logs.includes("spawn") || logs.includes("command not found") || logs.includes("couldn't find the binary") || logs.includes("is not recognized as an internal or external command") || logs.includes("err_pnpm_recursive_exec_first_fail");
}
function isMissingPackageManifest(data) {
  let logs = data.logs;
  return data.packageManagerErrorCode === "ENOENT" && logs.includes("package.json") || logs.includes("ERR_PNPM_NO_PKG_MANIFEST");
}
function isDependencyConflict(data) {
  return data.packageManagerErrorCode === "ERESOLVE";
}
function isPnpmIgnoredBuilds(data) {
  return data.packageManagerErrorCode === "ERR_PNPM_IGNORED_BUILDS" || data.logs.includes("Ignored build scripts");
}
function isPnpmNoTtyModulesDir(data) {
  return data.packageManagerErrorCode === "ERR_PNPM_ABORTED_REMOVE_MODULES_DIR_NO_TTY";
}
function categorizeExecaError(error, context) {
  if (error instanceof StorybookError)
    return error;
  let data = extractExecaCommandErrorData(error, context), { command, args } = data;
  return isAutomigrateAddonA11yTest(args) ? new AutomigrateAddonA11yTestError({ ...data, cause: error }) : isNuxtModuleAdd(args) ? new NuxtModuleAddFailedError({ ...data, cause: error }) : isPlaywrightInstall(args) ? new PlaywrightInstallFailedError({ ...data, cause: error }) : isBinaryNotFound(data) && isPackageManagerCommand(command) ? new PackageManagerBinaryNotFoundError({ ...data, cause: error }) : isPackageInstall(command, args) ? isDependencyConflict(data) ? new PackageInstallDependencyConflictError({ ...data, cause: error }) : isMissingPackageManifest(data) ? new PackageInstallMissingManifestError({ ...data, cause: error }) : isPnpmIgnoredBuilds(data) ? new PnpmIgnoredBuildsError({ ...data, cause: error }) : isPnpmNoTtyModulesDir(data) ? new PnpmNoTtyModulesDirError({ ...data, cause: error }) : new PackageInstallFailedError({ ...data, cause: error }) : new ExecaCommandFailedError({ ...data, cause: error });
}
...
function attachCategorizedExecaFailureHandler(execaProcess, context) {
  let categorize = (error) => categorizeExecaError(error, context), originalThen = execaProcess.then.bind(execaProcess), originalCatch = execaProcess.catch.bind(execaProcess);
  execaProcess.then = ((onFulfilled, onRejected) => originalThen(
    onFulfilled,
    typeof onRejected == "function" ? (error) => onRejected(categorize(error)) : (error) => {
      throw categorize(error);
    }
  )), execaProcess.catch = ((onRejected) => originalCatch((error) => {
    throw categorize(error);
  }).catch(typeof onRejected == "function" ? onRejected : void 0));
}
...
function executeCommand(options) {
  ...
  logger12.debug(`Executing command: ${command} ${args.join(" ")}`);
  ...
  }) : attachCategorizedExecaFailureHandler(execaProcess, { command, args }), execaProcess;
}
...
function isExecutableInPath(command) {
  return (process.env.PATH || "").split(delimiter).some((dir) => existsSync6(join10(dir, command)));
}
function tryCommandVariations(commandVariations, args, options) {
  ...
  for (let cmd of commandVariations.slice(0, -1))
    if (isExecutableInPath(cmd))
      return logger12.debug(`Resolved command variation: ${cmd}`), execa(cmd, args, options);
...
function tryCommandVariationsSync(commandVariations, args, options) {
  ...
  for (let i = 0; i < commandVariations.length; i++) {
    let cmd = commandVariations[i];
    try {
      let commandResult = execaCommandSync([cmd, ...args].join(" "), options);
      return typeof commandResult.stdout == "string" ? commandResult.stdout : "";
    } catch (error) {
      if (lastError = error, !shouldRetry(error, i === commandVariations.length - 1))
        throw error;
      logger12.debug(`Command "${cmd}" not found, trying next variation...`);
    }
  }
  throw lastError;
}
...
import { logger as logger13, prompt as prompt3 } from "storybook/internal/node-logger";
import {
...
var BUNProxy = class extends JsPackageManager {
  ...
  async precheckStorybookPackageInstall({
    ...
    if (nonInteractive) {
      this.updateMinimumReleaseAgeExcludes(), logger13.info(
    ...
    if (await prompt3.select(
      ...
      {
        onCancel: () => {
          throw logger13.error(rerunError.message), rerunError;
        }
      }
    ) === "exclude") {
...
`, replacement = [
    ...
    writeFileSync3(bunfigPath, nextContent);
  }
  ...
  readBunfig() {
    try {
      return readFileSync9(join11(this.cwd, "bunfig.toml"), "utf-8");
    } catch {
      return null;
    }
  }
...
import { logger as logger14, prompt as prompt4 } from "storybook/internal/node-logger";
import {
...
var NPMProxy = class extends JsPackageManager {
  ...
  async findInstallations(pattern, { depth = 99 } = {}) {
    let exec = ({ packageDepth }) => executeCommand({
      command: "npm",
      args: ["ls", "--json", `--depth=${packageDepth}`],
      env: {
        FORCE_COLOR: "false"
  ...
  async precheckStorybookPackageInstall({
    ...
    throw logger14.error(error.message), error;
  }
  async getRegistryURL() {
    let result = await executeCommand({
      command: "npm",
...
import { existsSync as existsSync7, readFileSync as readFileSync11, writeFileSync as writeFileSync4 } from "node:fs";
import { join as join13 } from "node:path";
import { pathToFileURL } from "node:url";
import { logger as logger15, prompt as prompt5 } from "storybook/internal/node-logger";
import {
  FindPackageVersionsError as FindPackageVersionsError3,
  MinimumReleaseAgeHandledError as MinimumReleaseAgeHandledError3
} from "storybook/internal/server-errors";
var import_ts_dedent12 = __toESM(require_dist(), 1), import_yaml = __toESM(require_dist2(), 1);
import { coerce as coerce2, gte } from "semver";
var PNPM_ALLOW_BUILD_DLX_MIN = "10.2.0", PNPMProxy = class extends JsPackageManager {
  constructor() {
    super(...arguments);
    this.type = "pnpm" /* PNPM */;
  }
  ...
  detectWorkspaceRoot() {
    ...
    return existsSync7(pnpmWorkspaceYaml);
  }
  getCommandName() {
    return "pnpm";
  }
  ...
  #readPnpmVersion() {
    if (this.#pnpmVersion !== void 0)
      return this.#pnpmVersion;
    try {
      logger15.debug("Executing command: pnpm --version");
      let stdout = executeCommandSync({
        cwd: this.cwd,
        command: "pnpm",
        args: ["--version"],
        stdio: "pipe"
  ...
  #pnpmGte(minimum) {
    let version = coerce2(this.#readPnpmVersion());
  ...
  async getModulePackageJSON(packageName) {
    ...
    if (pnpapiPath)
      try {
        ...
        return JSON.parse(
          readFileSync11(join13(pkg.packageLocation, "package.json"), "utf-8")
        );
      } catch (error) {
  ...
  async getDeclaredVersionSpecifier(packageName) {
    let specifier = await super.getDeclaredVersionSpecifier(packageName);
    if (specifier)
      return specifier;
    let catalogName = this.#getCatalogName(this.getAllDependencies()[packageName]);
    if (catalogName === null)
      return null;
    let workspace = this.#readWorkspaceYaml();
    if (!workspace)
      return null;
    let version = workspace.doc.getIn([
      ...this.#catalogKeyPath(workspace.doc, catalogName),
      packageName
    ]);
    return typeof version == "string" || typeof version == "number" ? String(version) : null;
  }
  applyVersionToRelatedPackages(packages, version, anchorPackage) {
    let catalogName = this.#getCatalogName(this.getAllDependencies()[anchorPackage]);
    return catalogName !== null && this.#registerCatalogEntries(packages, version, anchorPackage, catalogName) ? packages.map((pkg) => `${pkg}@catalog:${catalogName}`) : super.applyVersionToRelatedPackages(packages, version, anchorPackage);
  }
  /**
   ...
   */
  #getCatalogName(specifier) {
    let match = specifier?.match(/^catalog:(.*)$/);
   ...
   */
  #readWorkspaceYaml() {
    let path = up2("pnpm-workspace.yaml", {
      cwd: this.primaryPackageJson.operationDir,
      last: getProjectRoot()
    });
    if (!path)
      return null;
    try {
      let doc = (0, import_yaml.parseDocument)(readFileSync11(path, "utf8"));
      if (doc.errors.length > 0)
        throw doc.errors[0];
      return { path, doc };
    } catch (e) {
      return logger15.debug(`Could not read pnpm workspace file ${path}: ${String(e)}`), null;
    }
  }
  /**
   ...
   */
  #catalogKeyPath(doc, catalogName) {
    return catalogName && catalogName !== "default" ? ["catalogs", catalogName] : doc.hasIn(["catalogs", "default"]) ? ["catalogs", "default"] : ["catalog"];
  }
  /**
   ...
   */
  #registerCatalogEntries(packages, version, anchorPackage, catalogName) {
    let workspace = this.#readWorkspaceYaml();
    if (!workspace)
      return logger15.warn(
        `Could not read pnpm-workspace.yaml to register catalog entries for: ${packages.join(", ")}`
      ), !1;
    try {
      let keyPath = this.#catalogKeyPath(workspace.doc, catalogName), anchorVersion = workspace.doc.getIn([...keyPath, anchorPackage]), entryVersion = typeof anchorVersion == "string" || typeof anchorVersion == "number" ? String(anchorVersion) : version, changed = !1;
      for (let pkg of packages)
        workspace.doc.getIn([...keyPath, pkg]) === void 0 && (workspace.doc.setIn([...keyPath, pkg], entryVersion), changed = !0);
      return changed && writeFileSync4(workspace.path, workspace.doc.toString(), "utf8"), !0;
    } catch (e) {
      return logger15.warn(`Could not update pnpm catalog in ${workspace.path}: ${String(e)}`), !1;
    }
  }
  runInstall(options) {
    return executeCommand({
  ...
  async installDependencies(options) {
    try {
      ...
      if (logs.includes("ERR_PNPM_NO_MATURE_MATCHING_VERSION")) {
        ...
        throw logger15.error(handledError.message), handledError;
      }
      throw error;
    }
  }
  async precheckStorybookPackageInstall({
    ...
    if (nonInteractive) {
      await this.updateMinimumReleaseAgeExclude(), logger15.info(
    ...
    if (await prompt5.select(
      ...
      {
        onCancel: () => {
          throw logger15.error(rerunError.message), rerunError;
        }
      }
    ) === "exclude") {
...
import { logger as logger16, prompt as prompt6 } from "storybook/internal/node-logger";
...
var Yarn1Proxy = class extends JsPackageManager {
  ...
  getInstallArgs() {
    return this.installArgs || (this.installArgs = process2.env.CI ? [] : ["--ignore-workspace-root-check"]), this.installArgs;
  }
  getCommandName() {
    return "yarn";
  }
  ...
  async getModulePackageJSON(packageName) {
    ...
    return packageJsonPath ? JSON.parse(readFileSync12(packageJsonPath, "utf-8")) : null;
  }
  async getRegistryURL() {
    let childProcess = await executeCommand({
      command: "yarn",
  ...
  async findInstallations(pattern, { depth = 99 } = {}) {
    ...
    try {
      ...
      logger16.debug(`Error finding installations for ${pattern.join(", ")}: ${String(e)}`);
      return;
    }
  }
  getResolutions(packageJson, versions) {
...
var import_fslib = __toESM(require_lib(), 1), import_libzip = __toESM(require_sync(), 1);
...
var Yarn2Proxy = class extends JsPackageManager {
  ...
  async getModulePackageJSON(packageName) {
    ...
    return packageJsonPath ? JSON.parse(readFileSync13(packageJsonPath, "utf-8")) : null;
  }
  getResolutions(packageJson, versions) {
    return {
      resolutions: {
...
var JsPackageManagerFactory = class {
  ...
  static getPackageManagerType(cwd = process.cwd()) {
    let root = getProjectRoot(), closestLockfilePath = [
      ...
      let dirA = parse3(a).dir, dirB = parse3(b).dir, compare = relative5(dirA, dirB);
      return dirA === dirB ? 0 : compare.startsWith("..") ? -1 : 1;
    })[0], closestLockfile = closestLockfilePath && basename4(closestLockfilePath), yarnVersion = getYarnVersion(cwd);
...
function getYarnVersionFromPackageJson(cwd, root) {
  ...
  for (let dir of directories) {
    ...
    if (existsSync8(packageJsonPath))
      try {
        let packageManager = JSON.parse(readFileSync14(packageJsonPath, "utf-8")).packageManager;
...
import { logger as logger17, prompt as prompt8 } from "storybook/internal/node-logger";
async function scanAndTransformFiles({
  ...
  logger17.log("Scanning for affected files...");
  let globby = (await import("./globby-MBJFQ4QO.js")).globby, sourceFiles = await globby([glob2], {
    ...commonGlobOptions(""),
    ignore: ["**/node_modules/**"],
    dot: !0,
    cwd: getProjectRoot(),
  ...
  return logger17.log(`Scanning ${sourceFiles.length} files...`), transformFn(sourceFiles, transformOptions, dryRun);
}
...
var transformImportFiles = async (files, renamedImports, dryRun) => {
  let errors = [], { default: pLimit } = await import("./p-limit-OC7IM4Z4.js"), limit = pLimit(10);
  return await Promise.all(
    files.map(
      (file) => limit(async () => {
        try {
          let contents = await readFile7(file, "utf-8"), transformed = transformImports(contents, renamedImports);
...
// src/common/utils/file-lock.ts
import { logger as logger18 } from "storybook/internal/node-logger";
import { randomUUID } from "node:crypto";
import { readFileSync as readFileSync15, rmSync as rmSync2 } from "node:fs";
import { mkdir as mkdir2, open, readFile as readFile8, rm as rm4, stat, utimes } from "node:fs/promises";
import { dirname as dirname9 } from "node:path";
var POLL_INTERVAL_MS = 50, DEFAULT_STALE_AFTER_MS = 3e4, DEFAULT_WAIT_BUDGET_MS = 2e4, MALFORMED_LOCK_GRACE_MS = 5e3, errorCode = (error) => typeof error == "object" && error !== null && "code" in error ? error.code : void 0, delay = (ms) => new Promise((resolve9) => setTimeout(resolve9, ms)), isProcessAlive = (pid) => {
  try {
    return process.kill(pid, 0), !0;
  } catch (error) {
    return errorCode(error) !== "ESRCH";
  }
}, readPayload = async (lockPath) => {
  try {
    return JSON.parse(await readFile8(lockPath, "utf8"));
  } catch {
    return;
  }
}, tryAcquire = async (lockPath) => {
  await mkdir2(dirname9(lockPath), { recursive: !0 });
  let handle;
  try {
  ...
  let payload = { token: randomUUID(), pid: process.pid };
  try {
    await handle.writeFile(JSON.stringify(payload));
  } catch (error) {
    throw await handle.close(), await rm4(lockPath, { force: !0 }).catch(() => {
    }), error;
  }
  return await handle.close(), payload.token;
}, breakStaleLock = async (lockPath, staleAfterMs) => {
  let stats;
  try {
  ...
  let payload = await readPayload(lockPath), holderPid = typeof payload?.pid == "number" ? payload.pid : void 0, age = Date.now() - stats.mtimeMs;
  return (holderPid !== void 0 ? !isProcessAlive(holderPid) || age > staleAfterMs : age > MALFORMED_LOCK_GRACE_MS) ? ((await readPayload(lockPath))?.token !== payload?.token || (await rm4(lockPath, { force: !0 }), logger18.debug(`Cleared an abandoned lock at ${lockPath} (holder pid ${holderPid ?? "unknown"})`)), !0) : !1;
}, releaseIfOurs = (lockPath, token) => {
  try {
    JSON.parse(readFileSync15(lockPath, "utf8"))?.token === token && rmSync2(lockPath, { force: !0 });
  } catch {
  }
}, holdLock = (lockPath, token, staleAfterMs) => {
  let removeSync = () => releaseIfOurs(lockPath, token);
  ...
  return heartbeat.unref?.(), () => {
    clearInterval(heartbeat), process.off("exit", removeSync), removeSync();
  };
}, withFileLock = async (lockPath, run, {
  waitBudgetMs = DEFAULT_WAIT_BUDGET_MS,
  staleAfterMs = DEFAULT_STALE_AFTER_MS
} = {}) => {
  let deadline = Date.now() + waitBudgetMs;
  for (; ; ) {
    let token = await tryAcquire(lockPath);
    if (token !== void 0) {
      let release = holdLock(lockPath, token, staleAfterMs);
      try {
        return await run(), "ran";
      } finally {
        release();
      }
    }
    let retryImmediately = await breakStaleLock(lockPath, staleAfterMs);
    if (Date.now() >= deadline)
      return "busy";
    retryImmediately || await delay(POLL_INTERVAL_MS);
  }
};
