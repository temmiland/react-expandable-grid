...
var ANGULAR_JSON_PATH = "angular.json", ANALOG_VITE_PLUGIN_ANGULAR_VERSION = "^2.5.2", toDevkitVersion = (ngRange) => {
  if (!ngRange)
    return;
  let min = semver.validRange(ngRange) ? semver.minVersion(ngRange) : null;
  if (!min)
    return;
  let pre = min.prerelease.length > 0 ? `-${min.prerelease.join(".")}` : "", versionCore = `0.${min.major * 100 + min.minor}.${min.patch}${pre}`;
  return ngRange.trim().startsWith("^") ? `^${versionCore}` : versionCore;
}, detectIndentation = (text) => {
  let indent = /^[ \t]+(?=[^\s])/m.exec(text)?.[0];
  return indent ? indent.startsWith("	") ? { insertSpaces: !1, tabSize: 1 } : { insertSpaces: !0, tabSize: indent.length } : { insertSpaces: !0, tabSize: 2 };
}, editJsonText = (text, path2, value) => applyEdits(text, modify(text, path2, value, { formattingOptions: detectIndentation(text) })), isStorybookTarget = (target, builderPackage) => {
  if (typeof target != "object" || target === null)
    return !1;
  let ref = target.builder ?? target.executor;
  return typeof ref != "string" ? !1 : ["start-storybook", "build-storybook"].some(
    (command) => builderPackage ? ref === `${builderPackage}:${command}` : ref.endsWith(`:${command}`)
  );
}, AngularJSON = class {
