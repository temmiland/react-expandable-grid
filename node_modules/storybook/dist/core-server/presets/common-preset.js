...
import {
  REVIEW_EVENTS
} from "../../_node-chunks/chunk-XGT335GY.js";
import "../../_node-chunks/chunk-PNNIQSR4.js";
import "../../_node-chunks/chunk-MQGAF7D5.js";
import {
  GitDiffProvider,
  createDocsToolset,
...
import "../../_node-chunks/chunk-HFXQZX54.js";
import {
  registerToolset
} from "../../_node-chunks/chunk-5ZQGOLDE.js";
import "../../_node-chunks/chunk-5DGTSUE6.js";
import "../../_node-chunks/chunk-4C4AXOGG.js";
import "../../_node-chunks/chunk-G4ZCAUZF.js";
import "../../_node-chunks/chunk-LBGJYL6K.js";
import {
  get,
  isStoryCreatedByAISetup
} from "../../_node-chunks/chunk-XTZ2XLTY.js";
import "../../_node-chunks/chunk-ISBS76F6.js";
import "../../_node-chunks/chunk-ANRJD6WC.js";
import {
  sendTelemetryError
...
import "../../_node-chunks/chunk-PG6QDVJT.js";
import {
  getService,
  registerService,
  toMerged
...
var toExtractionError = (error) => error instanceof Error ? { name: error.name, message: error.message } : { name: "Error", message: String(error) };
...
function registerExtractionService(definition, options) {
    ...
    commands: {
      ...
      [extractAllCommand]: {
        handler: async (_input, ctx) => {
          let componentEntries = await resolveComponentEntries();
          await Promise.all(
            Array.from(
              componentEntries,
              ([id, entry]) => (
                ...
                // component's payload.
                extractComponent(ctx, id).catch((error) => {
                  let payload = buildErrorPayload({ id, entry, error: toExtractionError(error) });
                  ctx.self.setState((state) => {
                    state.components[id] = payload;
                  });
                })
  ...
  }), moduleGraph = getService("core/module-graph", { internal: !0 });
...
}), docgenOutputSchema = optional(docgenPayloadSchema), docgenServiceDef = defineService2({
  id: "core/docgen",
  internal: !0,
...
var DocgenWorker = class {
  constructor(scriptPath, descriptors, taskTimeoutMs = DEFAULT_TASK_TIMEOUT_MS) {
    ...
    this.nextId = 0;
    this.served = !1;
    this.worker = new Worker(scriptPath), this.worker.on("message", (msg) => this.handleMessage(msg)), this.worker.on("error", (error) => this.fail(error)), this.worker.on("exit", (code) => {
      this.death || this.fail(new Error(`docgen worker exited unexpectedly with code ${code}`));
    ...
    }), this.ready.finally(() => this.keepProcessAliveWhileBusy()).catch(() => {
   ...
   */
  get replaceable() {
    return this.death !== void 0 && this.served;
  }
  async extract(entry) {
    if (this.death)
    ...
    return await this.ready, new Promise((resolve2, reject) => {
      ...
      this.taskTimeoutMs > 0 && (pending.timer = setTimeout(() => {
        this.pending.delete(id), this.keepProcessAliveWhileBusy(), reject(new Error(`docgen worker extract ${id} timed out after ${this.taskTimeoutMs}ms`));
      }, this.taskTimeoutMs), pending.timer.unref?.()), this.pending.set(id, pending), this.keepProcessAliveWhileBusy(), this.post({ type: "extract", id, entry });
    });
  }
  keepProcessAliveWhileBusy() {
    this.pending.size > 0 ? this.worker.ref() : this.worker.unref();
  }
  post(msg) {
  ...
  handleMessage(msg) {
    ...
    pending && (pending.timer && clearTimeout(pending.timer), this.pending.delete(msg.id), this.served = !0, this.keepProcessAliveWhileBusy(), msg.error ? pending.reject(errorLikeToError(msg.error)) : pending.resolve(msg.payload));
  }
  fail(error) {
    this.death || (logger.warn(`docgen worker died, so component docs are unavailable: ${error.message}`), this.death = error, this.rejectReady(error), this.rejectAllPending(error), this.worker.terminate().catch(() => 0));
  }
...
function createDocgenWorkerClient(descriptors) {
  ...
  return {
    async extract(entry) {
      return worker?.replaceable && (logger.debug("docgen worker died after serving requests; spawning a replacement"), worker = void 0), worker ??= new DocgenWorker(scriptPath, descriptors), worker.extract(entry);
    }
  };
}

// src/shared/open-service/services/review/definition.ts
var reviewCollectionSchema = object({
...
}), REVIEW_STALE_GRACE_MS = 1e4, reviewServiceDef = defineService({
  id: "core/review",
  internal: !0,
...
// src/shared/open-service/services/review/state-transitions.ts
...
function applyPublishedReview(state, review) {
  state.current === null ? (state.current = review, state.pending = null) : state.pending = review;
}
function applyAcceptPending(state) {
  state.pending !== null && (state.current = toPlainReview(state.pending), state.pending = null);
}
function applyMarkStale(state, now) {
  let current = state.current;
  current?.createdAt !== void 0 && !current.stale && now >= current.createdAt + REVIEW_STALE_GRACE_MS && (state.current = { ...toPlainReview(current), stale: !0 });
}
function applyDismiss(state) {
  state.current = null, state.pending = null;
}

// src/shared/open-service/services/review/server.ts
var defaultSubscribeToModuleGraphChanges = (onChange) => {
  try {
    return getService("core/module-graph", { internal: !0 }).queries.graphRevision.subscribe(void 0, ({ data: revision }) => {
...
function registerReviewService({
  ...
  let service = registerService(reviewServiceDef, {
    commands: {
      setReview: {
        handler: async (input, ctx) => {
          let { stale: _stale, createdAt: _createdAt, ...review } = input, storyIds = [
            ...new Set(review.collections.flatMap((collection) => collection.storyIds))
          ], index = await getIndex(), unknownIds = storyIds.filter((storyId) => index.entries[storyId]?.type !== "story");
  ...
  return subscribeToModuleGraphChanges(() => {
    service.commands.markStale(void 0);
  }), service;
}

// src/shared/open-service/services/story-docs/definition.ts
import { defineService as defineService3 } from "storybook/open-service";
var storyDocsInputSchema = object({ id: string() }), storyDocsErrorSchema = object({
  ...
  error: optional(storyDocsErrorSchema)
}), storyDocsOutputSchema = optional(storyDocsPayloadSchema), storyDocsServiceDef = defineService3({
  id: "core/story-docs",
  internal: !0,
...
var isReviewFeatureEnabled = (features2) => features2?.experimentalReview !== !1 && !!features2?.changeDetection, isReviewExplicitlyEnabled = (features2) => !!features2?.experimentalReview && !!features2?.changeDetection;
...
async function getHeadlessChangeDetectionAdapter(options) {
  try {
    let core2 = await options.presets.apply("core", {});
    if (!core2?.builder)
      return;
    let resolvedPreviewBuilder = typeof core2.builder == "string" ? core2.builder : core2.builder.name;
    return await (await getPreviewBuilder(resolvedPreviewBuilder)).changeDetectionAdapter?.(options);
  } catch (error) {
    logger6.warn("Change detection: adapter initialisation failed"), logger6.debug(error instanceof Error ? error.stack ?? error.message : String(error));
    return;
  }
}
...
var services = async (_value, options) => {
  ...
  let features2 = await options.presets.apply("features"), storyIndex = { getIndex }, gitDiffProvider = new GitDiffProvider(process.cwd());
  if (registerToolset(
    createStoriesToolset({
      storyIndex,
      git: {
      ...
      // not point at it.
      reviewEnabled: isReviewExplicitlyEnabled(features2)
    })
  ), isReviewFeatureEnabled(features2) && (registerReviewService({
    getIndex
  }), registerToolset(reviewToolset)), features2?.experimentalDocgenServer && !options.ignorePreview) {
  ...
  let workingDir = process.cwd(), configDir = options.configDir, stories = await options.presets.apply("stories"), normalizedStories = normalizeStories(stories, {
    configDir,
    workingDir
  }), [indexers, docs, features2] = await Promise.all([
