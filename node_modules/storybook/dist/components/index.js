...
var languages = [], Comp = null, LazySyntaxHighlighter = lazy(async () => {
  let { SyntaxHighlighter: SyntaxHighlighter2 } = await import("../_browser-chunks/syntaxhighlighter-4REYKNEV.js");
  ...
  let [{ SyntaxHighlighter: SyntaxHighlighter2 }, { formatter }] = await Promise.all([
    import("../_browser-chunks/syntaxhighlighter-4REYKNEV.js"),
...
function createSlot(ownerName) {
  let Slot2 = React8.forwardRef((props, forwardedRef) => {
    let { children, ...slotProps } = props, slottableElement = null, hasSlottable = !1, newChildren = [];
    isLazyComponent(children) && typeof use == "function" && (children = use(children._payload)), React8.Children.forEach(children, (maybeSlottable) => {
    ...
    }), slottableElement ? slottableElement = React8.cloneElement(slottableElement, void 0, newChildren) : (
      // A `Slottable` was found but it didn't resolve to a single element (e.g.
      // it wrapped multiple elements, text, or a render-prop `child` that
      // wasn't an element). Don't fall back to treating the `Slottable` wrapper
      // itself as the slot target — throw a descriptive error below instead.
      !hasSlottable && React8.Children.count(children) === 1 && React8.isValidElement(children) && (slottableElement = children)
    );
    let slottableElementRef = slottableElement ? getElementRef(slottableElement) : void 0, composedRef = useComposedRefs(forwardedRef, slottableElementRef);
    if (!slottableElement) {
    ...
    let mergedProps = mergeProps(slotProps, slottableElement.props ?? {});
    return slottableElement.type !== React8.Fragment && (mergedProps.ref = forwardedRef ? composedRef : slottableElementRef), React8.cloneElement(slottableElement, mergedProps);
  });
  return Slot2.displayName = `${ownerName}.Slot`, Slot2;
}
var Slot = createSlot("Slot"), SLOTTABLE_IDENTIFIER = /* @__PURE__ */ Symbol.for("radix.slottable");
function createSlottable(ownerName) {
  let Slottable2 = (props) => "child" in props ? props.children(props.child) : props.children;
  return Slottable2.displayName = `${ownerName}.Slottable`, Slottable2.__radixId = SLOTTABLE_IDENTIFIER, Slottable2;
}
var Slottable = createSlottable("Slottable"), getSlottableElementFromSlottable = (slottable, child) => {
  if ("child" in slottable.props) {
    let child2 = slottable.props.child;
    return React8.isValidElement(child2) ? React8.cloneElement(child2, void 0, slottable.props.children(child2.props.children)) : null;
  }
...
var createSlotError = (ownerName) => `${ownerName} failed to slot onto its children. Expected a single React element child or \`Slottable\`.`, createSlottableError = (ownerName) => `${ownerName} failed to slot onto its \`Slottable\`. Expected \`Slottable\` to receive a single React element child.`, use = React8[" use ".trim().toString()];
...
var DEFAULT_MAX_WIDTH = 260, Note = styled26.div(
  ({ theme }) => ({
    fontFamily: theme.typography.fonts.base,
    padding: "2px 6px",
    lineHeight: "16px",
  ...
  ({ maxWidth }) => ({ maxWidth })
), TooltipNote = ({ note, maxWidth = DEFAULT_MAX_WIDTH, ...props }) => React9.createElement(Note, { maxWidth, ...props }, note);
...
var { document: document3 } = global2, ErrorName = styled37.strong(({ theme }) => ({
  ...
  if (!error)
    return React25.createElement(Fragment2, null, "This error has no stack or message");
  if (!error.stack)
    return React25.createElement(Fragment2, null, error.message || "This error has no stack or message");
...
${input}`);
  ...
  if (!match)
    return React25.createElement(Fragment2, null, input);
  ...
  return React25.createElement(Fragment2, null, React25.createElement("span", null, type), ": ", React25.createElement(ErrorName, null, name), React25.createElement("br", null), lines.map(
    (l, i) => l?.name ? React25.createElement(Fragment2, { key: i }, "  ", "at ", React25.createElement(ErrorImportant, null, l.name), " (", React25.createElement(ErrorDetail, null, l.location), ")", React25.createElement("br", null)) : React25.createElement(Fragment2, { key: i }, "  ", "at ", React25.createElement(ErrorDetail, null, l?.location), React25.createElement("br", null))
  ));
};
...
import React43, { Fragment as Fragment3, useCallback as useCallback10 } from "react";
...
var List = styled50.div(
  ...
  return React43.createElement(List, { "data-deprecated": "TooltipLinkList", ...props, className: "sb-list" }, groups.filter((group) => group.length).map((group, index3) => React43.createElement(Group, { key: group.map((link) => link.id).join(`~${index3}~`) }, group.map((link) => "content" in link ? React43.createElement(Fragment3, { key: link.id }, link.content) : React43.createElement(Item2, { key: link.id, isIndented, LinkWrapper, ...link })))));
};
...
function TabsImpl({
  ...
  return !showToolsWhenEmpty && list.length === 0 ? EmptyContent : React49.createElement(Wrapper5, { "data-deprecated": "Tabs", absolute, bordered, id: htmlId }, React49.createElement(FlexBar, { scrollable: !1, border: !0, backgroundColor }, React49.createElement(TabBar, { style: { whiteSpace: "normal" }, ref: tabBarRef, role: "tablist" }, visibleList.map(({ title, id, active, color: color4 }, index3) => {
    let indexId = `index-${index3}`;
    return (
...
import React50, { Fragment as Fragment4 } from "react";
...
var interleaveSeparators = (list) => list.reduce(
  (acc, item, index3) => item ? React50.createElement(Fragment4, { key: item.id || item.key || `f-${index3}` }, acc, index3 > 0 ? React50.createElement(Separator2, { key: `s-${index3}` }) : null, item.render() || item) : acc,
  null
);
